...
public sealed partial class HostController
{
    ...
    public ImportPlan BuildImportPlan(string path)
    {
        ...
        return ext switch
        {
            ".vrm" => new ImportPlan("vrm", true, "VRM 경로가 선택되었습니다. 기본 런타임 렌더 경로를 사용합니다.", "호환성 경고가 반복되면 MIQ로 변환해 다시 시도하세요."),
            ".miq" => new ImportPlan("miq", true, "MIQ 경로가 선택되었습니다. vxa2 기반 컨테이너를 사용합니다.", "불러오기가 막히면 VRM 경로로 재시도하세요."),
            ...
            _ => new ImportPlan("unsupported-extension", false, "지원하지 않는 확장자입니다. 지원 형식: .vrm, .vsfavatar, .miq", "지원 형식으로 변환한 뒤 다시 시도하세요."),
        };
    }
    ...
    public PreflightSummary RunPreflight()
    {
        ...
        if (!string.IsNullOrWhiteSpace(localAppData))
        {
            try
            {
                ...
                var probeFile = Path.Combine(localAppData, $"animiq_preflight_{Guid.NewGuid():N}.tmp");
        ...
        var diagnosticsPath = Path.Combine(localAppData, "AnimiqHost", "diagnostics");
    ...
    public string ExportDiagnosticsBundle(string outputDirectory)
    {
        var root = string.IsNullOrWhiteSpace(outputDirectory)
            ? Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "AnimiqHost", "diagnostics")
            : outputDirectory;
    ...
    private string BuildDiagnosticsReproCommands()
    {
        ...
        sb.AppendLine("Animiq Host Repro Commands");
        ...
        sb.AppendLine("$env:ANIMIQ_MEDIAPIPE_PYTHON='C:\\\\path\\\\to\\\\python.exe'; powershell -ExecutionPolicy Bypass -File .\\tools\\release_readiness_gate.ps1 -RenderPerfProfile desktop-60");
        sb.AppendLine();
    ...
    private static object BuildEnvironmentSnapshot()
    {
        var envSubset = new Dictionary<string, string?>(StringComparer.OrdinalIgnoreCase)
        {
            ...
            ["ANIMIQ_MEDIAPIPE_PYTHON"] = Environment.GetEnvironmentVariable("ANIMIQ_MEDIAPIPE_PYTHON"),
    ...
    public string ExportRollingMetricsCsv(string outputPath)
    {
        var path = string.IsNullOrWhiteSpace(outputPath)
            ? Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "AnimiqHost", "metrics", $"metrics_{DateTimeOffset.UtcNow:yyyyMMdd_HHmmss}.csv")
            : outputPath;
    ...
    public string GetReleaseTrackStatus()
    {
        try
        {
            var candidates = new[]
            {
                ...
                Path.Combine(Directory.GetCurrentDirectory(), "AnimiqCore", "build", "reports", "winui", "winui_diagnostic_manifest.json"),
            };
    ...
    {
        var path = string.IsNullOrWhiteSpace(outputPath)
            ? Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "AnimiqHost", "telemetry", $"telemetry_{DateTimeOffset.UtcNow:yyyyMMdd_HHmmss}.json")
            : outputPath;
    ...
    private UserFacingError BuildUserFacingError(string source, NcResultCode rc, string detail)
    {
        ...
        if (source.Contains("LoadAvatar", StringComparison.OrdinalIgnoreCase))
        {
            ...
            {
                title = $"Runtime avatar load failed (format recognized, stage={parserStage})";
                action = "Check parser_stage/primary_error details, then retry. If it fails again, export diagnostics and convert to .miq/.vrm.";
            }
