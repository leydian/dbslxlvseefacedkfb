...
public sealed partial class HostController
{
    ...
    private static bool TryValidateAvatarPath(string path, out string error)
    {
        ...
        {
            error = "Unsupported avatar file extension. Supported: .vrm, .vsfavatar, .miq. If your file is .xva2, rename/re-export to .miq.";
            return false;
        }
    ...
    private void AutoReconfigureSpout(uint width, uint height)
    {
        ...
        var channel = string.IsNullOrWhiteSpace(Outputs.SpoutChannelName) ? "Animiq" : Outputs.SpoutChannelName;
