...
public sealed partial class MainWindow : Window
{
    ...
    private async void Load_Click(object sender, RoutedEventArgs e)
    {
        ...
        {
            _recoveryHint = "런타임 경로 불일치(nativecore). dist/wpf 빌드로 실행 후 다시 시도하세요.";
        ...
        {
            _recoveryHint = "런타임 nativecore가 빌드 출력보다 오래되었습니다. dist를 재배포 후 다시 시도하세요.";
            await ShowMessageAsync(
                "불러오기 차단 (Load Blocked)",
    ...
    private string BuildTrackingStartFailureMessage(NcResultCode rc)
    {
        ...
        {
            "TRACKING_MEDIAPIPE_CONFIG_INVALID" => $"{Environment.NewLine}Action: verify mediapipe_webcam_sidecar.py path and ANIMIQ_MEDIAPIPE_SIDECAR_SCRIPT.",
            "TRACKING_MEDIAPIPE_START_FAILED" => $"{Environment.NewLine}Action: set ANIMIQ_MEDIAPIPE_PYTHON or run tools/setup_tracking_python_venv.ps1.",
