...
function Resolve-PythonExecutable {
    ...
    $envValue = [Environment]::GetEnvironmentVariable("ANIMIQ_MEDIAPIPE_PYTHON")
    if (-not [string]::IsNullOrWhiteSpace($envValue)) {
        return [PSCustomObject]@{
            Executable = $envValue.Trim()
            Source = "env:ANIMIQ_MEDIAPIPE_PYTHON"
            IsExplicit = $true
        }
    }
...
if ([string]::IsNullOrWhiteSpace($resolvedPythonExe)) {
    $checks.Add("- python_executable: FAIL (missing explicit python executable; set ANIMIQ_MEDIAPIPE_PYTHON or pass -PythonExe)")
    $overall = $false
} else {
    try {
