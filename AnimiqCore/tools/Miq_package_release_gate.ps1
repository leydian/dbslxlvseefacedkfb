param(
    [string]$PackageRoot = ".\unity\Packages\com.animiq.miq",
...
function Assert-PathExists {
    ...
    if (-not (Test-Path -LiteralPath $Path)) {
        throw "[miq_package_release_gate] missing_required_file label=$Label path=$Path"
    }
}
...
if (-not $pkg.documentationUrl) {
    throw "[miq_package_release_gate] package.json missing documentationUrl"
}
if (-not $pkg.changelogUrl) {
    throw "[miq_package_release_gate] package.json missing changelogUrl"
}
if (-not $pkg.licensesUrl) {
    throw "[miq_package_release_gate] package.json missing licensesUrl"
}
if (-not $pkg.repository.url) {
    throw "[miq_package_release_gate] package.json missing repository.url"
}
if (-not $pkg.samples -or $pkg.samples.Count -lt 2) {
    throw "[miq_package_release_gate] package.json must define at least two samples"
}
...
Write-Host "[miq_package_release_gate] PASS package=$($pkg.name) version=$($pkg.version)"
exit 0
