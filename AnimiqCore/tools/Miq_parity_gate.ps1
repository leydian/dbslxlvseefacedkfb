...
$unityLogPath = Join-Path $ReportDir "unity_miq_${ReportSuffix}_parity_gate.log"
$unityReportPath = Join-Path $ReportDir "unity_miq_${ReportSuffix}_parity_probe.json"
$paritySamplesDir = Join-Path $ReportDir "miq_parity_samples_${ReportSuffix}"
$summaryTxtPath = Join-Path $ReportDir "miq_parity_gate_${ReportSuffix}_summary.txt"
$summaryJsonPath = Join-Path $ReportDir "miq_parity_gate_${ReportSuffix}_summary.json"
...
$args = @(
    ...
    "-executeMethod", "Animiq.Miq.Editor.MiqCiQuality.RunParityProbe",
    "-miqParityReportPath", $unityReportPath,
    "-miqParityOutputDir", $paritySamplesDir,
    "-miqExpectedUnityVersion", $ExpectedUnityVersion,
    "-miqParityMaxExternalSamples", "$MaxExternalSamples",
    "-logFile", $unityLogPath
)
if (-not [string]::IsNullOrWhiteSpace($SampleDir)) {
    $args += @("-miqParitySampleDir", $SampleDir)
}
...
$lines = @()
$lines += "MIQ Unity/Native Parity Gate Summary"
...
if ($UnityLine -eq "2021-lts") {
    Copy-Item -Force -Path $unityLogPath -Destination (Join-Path $ReportDir "unity_miq_parity_gate.log")
    Copy-Item -Force -Path $unityReportPath -Destination (Join-Path $ReportDir "unity_miq_parity_probe.json")
    Copy-Item -Force -Path $summaryJsonPath -Destination (Join-Path $ReportDir "miq_parity_gate_summary.json")
    Copy-Item -Force -Path $summaryTxtPath -Destination (Join-Path $ReportDir "miq_parity_gate_summary.txt")
}
