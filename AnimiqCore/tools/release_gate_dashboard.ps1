...
$unityLtsGateJson = Join-Path $ReportDir "unity_miq_lts_gate_summary.json"
...
$unityLtsKpiJson = Join-Path $ReportDir "unity_miq_lts_kpi_summary.json"
...
$unityValidationSummary = Join-Path $ReportDir "unity_miq_validation_summary.txt"
...
$rows += [PSCustomObject]@{
    track = "Unity MIQ Validate"
    status_line = $unityStatus
    source_file = $unityValidationSummary
}

$unityLtsGate = Join-Path $ReportDir "unity_miq_lts_gate_summary.txt"
...
$rows += [PSCustomObject]@{
    track = "Unity MIQ LTS Gate (Overall)"
...
if ($unityLtsSummary -ne $null -and $unityLtsSummary.line_status -ne $null) {
    foreach ($lineRow in $unityLtsSummary.line_status) {
        $rows += [PSCustomObject]@{
            track = "Unity MIQ LTS Line [$($lineRow.line)]"
...
if ($unityLtsKpiSummary -ne $null -and $unityLtsKpiSummary.rows -ne $null) {
    foreach ($kpiRow in $unityLtsKpiSummary.rows) {
        $rows += [PSCustomObject]@{
            track = "Unity MIQ LTS KPI [$($kpiRow.line)]"
...
$compressionGate = Join-Path $ReportDir "miq_compression_quality_gate_summary.txt"
$rows += [PSCustomObject]@{
    track = "MIQ Compression Quality"
    status_line = Get-StatusFromFile -Path $compressionGate -Pattern "- Overall:"
    source_file = $compressionGate
}

$parityGate = Join-Path $ReportDir "miq_parity_gate_summary.txt"
$rows += [PSCustomObject]@{
    track = "MIQ Unity/Native Parity"
...
$compressionPass = (Get-PassFailFromStatusLine -Line ((($rows | Where-Object { $_.track -eq "MIQ Compression Quality" }) | Select-Object -First 1).status_line)) -eq "PASS"
$parityPass = (Get-PassFailFromStatusLine -Line ((($rows | Where-Object { $_.track -eq "MIQ Unity/Native Parity" }) | Select-Object -First 1).status_line)) -eq "PASS"
...
$unityMiqAllPass = switch ($unityLtsState) {
    "PASS" { $true }
    "FAIL" { $false }
...
$wpfUnityRequirementMet = if ($RequireUnityMiqForWpfOnly) { $unityMiqAllPass } else { $true }
$fullUnityRequirementMet = if ($RequireUnityMiqForFull) { $unityMiqAllPass } else { $true }
$wpfUnityRecentRiskRequirementMet = if ($RequireUnityMiqRecentRiskZeroForWpfOnly) { -not $unityLtsRecentRisk } else { $true }
...
if ($hostTrack.wpf_state -ne "PASS") { $wpfFailureReasons.Add("HOST_WPF_FAIL") }
if (-not $wpfUnityRequirementMet) { $wpfFailureReasons.Add("UNITY_MIQ_FAIL") }
...
if ($hostTrack.wpf_state -ne "PASS") { $fullFailureReasons.Add("HOST_WPF_FAIL") }
if ($hostTrack.winui_state -ne "PASS") { $fullFailureReasons.Add("HOST_WINUI_FAIL_OR_SKIP") }
if (-not $fullUnityRequirementMet) { $fullFailureReasons.Add("UNITY_MIQ_FAIL") }
...
$summary = [PSCustomObject]@{
    ...
    gate_summary = [ordered]@{
        ...
        unity_miq_validate_pass = $unityPass
        unity_miq_lts_gate_pass = $unityLtsPass
        unity_miq_lts_recent_risk = $unityLtsRecentRisk
        unity_miq_lts_recent_risk_lines = $unityLtsRecentRiskLines
        miq_compression_quality_pass = $compressionPass
        miq_unity_native_parity_pass = $parityPass
        unity_miq_all_pass = $unityMiqAllPass
        unity_miq_required_wpf_only = [bool]$RequireUnityMiqForWpfOnly
        unity_miq_required_full = [bool]$RequireUnityMiqForFull
        unity_miq_recent_risk_zero_required_wpf_only = [bool]$RequireUnityMiqRecentRiskZeroForWpfOnly
        unity_miq_recent_risk_zero_required_full = [bool]$RequireUnityMiqRecentRiskZeroForFull
        unity_miq_recent_risk_zero_requirement_met_wpf_only = [bool]$wpfUnityRecentRiskRequirementMet
        unity_miq_recent_risk_zero_requirement_met_full = [bool]$fullUnityRecentRiskRequirementMet
        ...
        release_candidate_wpf_only_reasons = @($wpfFailureReasons)
        release_candidate_full_reasons = @($fullFailureReasons)
    }
    rows = $rows
}
...
$lines += "Policy.RequireUnityMiqForWpfOnly: $RequireUnityMiqForWpfOnly"
$lines += "Policy.RequireUnityMiqForFull: $RequireUnityMiqForFull"
$lines += "Policy.RequireUnityMiqRecentRiskZeroForWpfOnly: $RequireUnityMiqRecentRiskZeroForWpfOnly"
...
$lines += "UnityMiqLtsRecentRiskLines: $(if ($unityLtsRecentRiskLines.Count -gt 0) { $unityLtsRecentRiskLines -join ', ' } else { '<none>' })"
...
$lines += "ReleaseCandidateWpfOnlyReasons: $(if ($wpfFailureReasons.Count -gt 0) { $wpfFailureReasons -join ', ' } else { '<none>' })"
$lines += "ReleaseCandidateFull: $(if ($fullReleaseCandidate) { 'PASS' } else { 'FAIL' })"
$lines += "ReleaseCandidateFullReasons: $(if ($fullFailureReasons.Count -gt 0) { $fullFailureReasons -join ', ' } else { '<none>' })"
$lines += ""
foreach ($r in $rows) {
