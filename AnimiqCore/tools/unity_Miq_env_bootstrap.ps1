param(
    ...
    [string]$ReportPath = ".\build\reports\unity_miq_env_bootstrap_summary.txt",
    [string]$ReportJsonPath = ".\build\reports\unity_miq_env_bootstrap_summary.json"
)
...
if ($projectExists) {
    [System.Environment]::SetEnvironmentVariable("UNITY_MIQ_PROJECT_PATH", $projectResolved, "Process")
...
if ($PersistUserEnv) {
    ...
    if ($projectExists) {
        [System.Environment]::SetEnvironmentVariable("UNITY_MIQ_PROJECT_PATH", $projectResolved, "User")
        $setUserProject = $true
    }
}
...
$summary = [ordered]@{
    ...
    hints = @(
        "$editorEnvVar and UNITY_MIQ_PROJECT_PATH are both required by Unity MIQ gate scripts for line $UnityLine.",
        "If project path is missing, set UNITY_MIQ_PROJECT_PATH to a Unity project root containing ProjectSettings/ProjectVersion.txt."
    )
}

$lines = @()
$lines += "Unity MIQ Env Bootstrap Summary"
...
$lines += ""
$lines += "Hints:"
$lines += "- $editorEnvVar and UNITY_MIQ_PROJECT_PATH are both required by Unity MIQ gates for line $UnityLine."
$lines += "- If project path is missing, point UNITY_MIQ_PROJECT_PATH to a Unity project root (contains ProjectSettings/ProjectVersion.txt)."
