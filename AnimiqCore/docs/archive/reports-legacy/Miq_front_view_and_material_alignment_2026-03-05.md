# MIQ Front-View Orientation and Material Alignment Update (2026-03-05)

## Summary

This update closes the latest MIQ visual mismatch loop for `개인작11-3.miq` by addressing:
...
- MIQ now opens in front-view orientation by default in WPF host preview.
...
- For MIQ assets in this pipeline, that resulted in default back-view presentation.

### B) Material/texture alignment

- In MIQ loader, when material payload had empty `base_color_texture_name`, code assigned `textureRefs.front()`.
...
### 1) MIQ-specific front-view orientation
...
- `preview_yaw` is now format-aware in preview transform:
  - `MIQ` -> `0.0f` (front view default)
  - others -> existing `PI` path preserved

### 2) Removed unsafe texture fallback in MIQ loader
