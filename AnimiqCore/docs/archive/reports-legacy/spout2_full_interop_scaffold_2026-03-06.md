...
- Added `ANIMIQ_ENABLE_SPOUT2` CMake option (`ON` by default).
- Added SDK detection logic at `third_party/Spout2/include`.
- Added conditional compile definition `ANIMIQ_SPOUT2_ENABLED=1` when SDK include path is detected.
...
- `include/animiq/stream/spout_sender_stub.h`
...
- reads `ANIMIQ_ENABLE_SPOUT2` from CMake cache
...
- CMake option `-DANIMIQ_ENABLE_SPOUT2=ON`
...
```powershell
cmake -S NativeAnimiq -B NativeAnimiq\build -G "Visual Studio 17 2022" -A x64 -DANIMIQ_ENABLE_SPOUT2=ON
```
