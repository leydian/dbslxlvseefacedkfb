# MIQ Visibility Fallback and Warning-Gate Update (2026-03-06)

## Summary

This update addresses the "load succeeds but avatar not visible" troubleshooting line for `.miq` by hardening native v3 skinning validation/fallback and improving warning diagnostics visibility in both tooling and host UI.
...
- MIQ render regression gate now evaluates full warning-code set for critical warning detection
...
- `.miq` file loads to `runtime-ready` with `PrimaryError=NONE`
...
- `BuildGpuMeshForPayload(...)`
  ...
  - behavior:
    ...
    - env opt-in (`ANIMIQ_MIQ_ENABLE_STATIC_SKINNING`) still works as before
...
- This update focuses on visibility-stability and diagnostics correctness for MIQ runtime/gate/tool/host diagnostics.
