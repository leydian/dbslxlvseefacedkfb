...
This review defines 20 platform requirements for `NativeAnimiq` and evaluates each item across five personas:
...
| R02 | Unified avatar import wizard for `.vrm/.vxavatar/.vxa2/.miq/.vsfavatar` | Loader/Host | one path to success | faster switching | fast fallback in emergencies | lower support cost | shared loader facade reuse | 5 | 4 | 4 | Short-term | +1 release |
...
| R15 | Unity MIQ export validator and compatibility checker | Unity SDK | fewer bad exports | faster iteration loop | fewer runtime import failures | strengthens ecosystem | editor/runtime contract tests | 4 | 4 | 3 | Medium-term | +2 releases |
...
- Scope includes full platform (`NativeCore`, loader stack, Host apps, Unity MIQ SDK, release/ops tooling).
