# MIQ VRM-origin detached-cluster hotfix + bust focus retune (2026-03-06)

## Scope

- This report covers a follow-up runtime hotfix for the case where `sourceExt=.vrm` MIQ avatars still showed:
...
- Static skinning policy rollback for VRM-origin MIQ:
...
- VRM-origin draw guard hardening:
  - in draw queue build, if avatar is MIQ with `sourceExt=.vrm`:
...
- VRM-origin bust focus retune:
  - under `AutoFitBust`, MIQ+VRM-origin adds a stronger robust-center pull:
...
- This hotfix intentionally biases stability over maximum accessory retention for VRM-origin MIQ.

## Next Steps

1. Run the target avatar through host runtime and check warning list for `MIQ_VRM_ORIGIN_DETACHED_CLUSTER_SKIPPED`.
2. If detached skips are excessive, tune VRM-origin size cap first (`max(1.8, median_extent * 3.2)`).
3. Add targeted regression samples to `miq_render_regression_gate` corpus for sustained guard coverage.
