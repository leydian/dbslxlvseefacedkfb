...
- Newly added: review `2026-03-08_vrm_wpf_render_recovery_and_loader_index_contract_fix.md` for WPF `RenderRc=Internal` recovery hardening, thumbnail-worker based failure isolation (`gpu mesh build invalid input`), and final VRM loader primitive `indices/material` contract restoration for full render recovery.
...
- Newly added: review `2026-03-08_vnyan_10axis_benchmark_execution_pack.md` for `animiq vs vnyan` 10-axis benchmark harness, KPI schema contracts, automated scorecard generation, and 4-week execution acceptance criteria.
- Newly added: review `2026-03-08_wpf_arm_pose_vseeface_benchmark_alignment_and_redeploy.md` for arm slider stabilization via local-space hierarchy recomposition, VSeeFace-like upperarm/twist policy (`1.00/0.35/0.00`), sleeve follow recovery guard tuning, and WPF redeploy integrity verification.
- Newly added: review `2026-03-08_automation_workflow_and_spout2_receiver_mvp.md` for HostCore workflow runtime (`timer/state/osc -> action`), WPF automation panel integration, native Spout2 receiver API contract extension, and end-to-end build verification evidence.
- Newly added: review `2026-03-08_vnyan_10axis_mvp_feature_implementation.md` for Command/OSC/BatchExpression/Overlay automation node expansion, Spout receiver auto-reconnect path, WPF Quick Preset + Panic Stop + Recovery controls, and automation extension registry contract.
