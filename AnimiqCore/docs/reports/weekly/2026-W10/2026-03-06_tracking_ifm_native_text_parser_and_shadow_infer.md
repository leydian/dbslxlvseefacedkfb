# Tracking IFM Native Text Parser and MIQ Base-Only Shadow Inference (2026-03-06)

## Summary

This change resolves the live iFacial receive failure where packets arrived but were classified as parse failures, and also hardens native MIQ shadow-pass inference for legacy base-only pass declarations.
...
4. Native renderer infers shadow pass for parity-family MIQ materials when pass flags are base-only, preventing shadow dropout on legacy exports.
...
- In parallel, certain MIQ exports with base-only pass declarations could miss shadow path enablement despite valid parity-family context.
...
### 4) MIQ base-only pass shadow inference (NativeCore)
...
- For MIQ parity-family materials, infer shadow pass enablement under this condition:
  - `material.enable_shadow_pass = true`
  - fallback reason: `miq_pass_flags_base_only_shadow_inferred`

Result:

- Legacy MIQ exports with sparse pass declarations retain expected shadow caster behavior.
...
- Native shadow inference is scoped to MIQ + parity-family + base-only pass declarations to avoid broad side effects.
