# MIQ Unity LTS Matrix Gate Policy Update (2026-03-06)

## Summary

Implemented matrix-driven Unity LTS gate orchestration for MIQ with
official-line blocking policy and release dashboard integration.

## Implemented Changes

- `tools/unity_miq_lts_gate.ps1` (new/updated)
...
- orchestrates per-line execution of:
    - `unity_miq_validate.ps1`
    - `miq_parity_gate.ps1`
    - `miq_compression_quality_gate.ps1`
...
- `tools/run_quality_baseline.ps1`
  - `-EnableUnityMiqLtsGate` default set to enabled
- `tools/release_readiness_gate.ps1`
  - `-EnableUnityMiqLtsGate` default set to enabled and passed through to quality baseline
- `tools/release_gate_dashboard.ps1`
  - added `Unity MIQ LTS Gate` row
  - release decision now hard-fails Unity MIQ status when LTS gate is `FAIL`
  - fallback to legacy single-line signals only when LTS gate is missing/unknown
  - added recent-pass-rate risk signal sourced from `unity_miq_lts_kpi_summary.json`
...
- PowerShell parser check PASS for updated scripts:
  - `miq_parity_gate.ps1`
  - `unity_miq_lts_gate.ps1`
  - `unity_miq_env_bootstrap.ps1`
