# MIQ outlier draw policy default change (`autofit_only`) + runtime DLL mismatch recovery (2026-03-06)

## Summary

Resolved the user-facing regression where shoes/accessory meshes disappeared after `.miq` import by separating camera AutoFit outlier filtering from draw exclusion policy.
...
1. MIQ preview outlier filtering was used for both:
...
- Added `ANIMIQ_MIQ_OUTLIER_DRAW_POLICY` resolution logic.
- Supported values:
...
- Updated `docs/formats/miq.md` to document:
  - new env variable
...
- Env check:
  - `ANIMIQ_MIQ_OUTLIER_DRAW_POLICY` unset -> default `autofit_only` path active.
...
3. relaunch host and retest target `.miq` samples
