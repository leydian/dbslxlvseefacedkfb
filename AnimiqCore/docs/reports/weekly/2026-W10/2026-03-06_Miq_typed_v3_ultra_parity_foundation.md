# MIQ typed-v3 + UltraParity foundation implementation report (2026-03-06)

## Summary

이번 변경은 `.miq`의 "최상급 품질/완전 셰이더 재현" 단계로 가기 위한 기반 공사에 해당한다.
핵심은 다음 4축이다.
...
### 1) MIQ typed-v3 schema foundation (native + Unity)
...
- render perf gate profile 추가:
  ...
  - `ultra-parity` profile 기본 임계치 추가
    ...
    - `P99 <= 20.0`
    - `DropRatio <= 0.01`
- MIQ render regression gate 강화:
  - `tools/miq_render_regression_gate.ps1`
  - snapshot parity 필수화 옵션 `-RequireSnapshotParity` 추가
  - `MIQ_SHADER_PARITY_*` warning code를 critical warning 판정 대상으로 포함
- format 문서 갱신:
  - `docs/formats/miq.md`
