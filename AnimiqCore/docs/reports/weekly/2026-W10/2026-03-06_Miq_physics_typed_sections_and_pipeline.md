# MIQ Physics Typed Sections and Pipeline Rollout (2026-03-06)

## Scope

This report covers the first implementation slice for transporting VRM/VRChat physics metadata through `.miq` using typed sections and keeping Unity/native loader behavior aligned.

In scope:

- MIQ format and manifest extension for physics metadata
- Unity exporter/extractor/importer/runtime loader support
- Native MIQ loader contract and parser updates
...
- Added payload models:
  - `MiqPhysicsColliderPayload`
  - `MiqSpringBonePayload`
  - `MiqPhysBonePayload`
- Extended `MiqManifest` and `MiqAvatarPayload` with physics fields/collections.
...
- Added native payload structs:
  ...
  - `SpringBonePayload`
  - `PhysBonePayload`
- Added MIQ parser support for physics sections.
...
- Outcomes:
  - Native build: PASS
  - Sample `.miq` load via `avatar_tool`: PASS
    - `Compat=full`
