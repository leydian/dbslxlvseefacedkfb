...
2. Unity MIQ gate inputs and execution path not green
...
### 2) MIQ gate corpus and gate run recheck

Executed:

- `tools/miq_prepare_gate_corpus.ps1 -SourceDir . -OutputDir .\build\gate_corpus\miq -MinSampleCount 10 -IncludeBuildArtifacts`
- `tools/miq_render_regression_gate.ps1` with corpus + manifest inputs
...
3. Unity MIQ full-chain gates are still not green in dashboard status.
...
2. Provide a valid `UNITY_MIQ_PROJECT_PATH` and execute Unity MIQ validate/compression/parity tracks in one lane.
