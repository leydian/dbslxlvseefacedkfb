# MIQ lilToon Texture Export Fallback Update (2026-03-05)

## Summary

This update addresses a post-export rendering symptom where `.miq` avatars exported from Unity (including lilToon-based avatars) appeared as flat silhouette-like output with missing surface texture detail.

Observed behavior:

- Export succeeded and `.miq` file loaded.
...
- non-readable source textures can still be serialized into `.miq` texture payloads.
...
- `[MIQ] Texture encode failed for material '{material}', texture '{texture}'.`
