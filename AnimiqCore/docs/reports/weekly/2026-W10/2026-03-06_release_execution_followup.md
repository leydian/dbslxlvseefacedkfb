...
- repeatedly invokes `avatar_tool.exe` across `.vrm/.miq/.vsfavatar` samples
...
- updated `src/nativecore/native_core.cpp`:
  - force no-cull raster state for MIQ source meshes (`AvatarSourceType::Miq`)
...
powershell -ExecutionPolicy Bypass -File .\tools\avatar_load_soak_gate.ps1 -SampleDir .. -IterationsPerSample 1 -IncludePatterns "*.miq"
...
### A) MIQ SDK quality and compression track

- Export options/API expansion:
  - `MiqExportOptions` now includes compression controls:
    - `EnableCompression`
...
- Native loader parity:
  - native MIQ loader now accepts v5 and decodes compressed sections with equivalent LZ4 path
...
- `docs/formats/miq.md` updated to include:
...
- dashboard text/json output now includes `Unity MIQ Validate` row.
...
- Added Unity CI quality entrypoint:
  - `unity/Packages/com.animiq.miq/Editor/MiqCiQuality.cs`
  - methods:
    - `RunCompressionGate`
    - `RunParityProbe`
- Added compression KPI gate script:
  ...
  - outputs:
    - `build/reports/miq_compression_quality_gate_summary.txt`
    - `build/reports/miq_compression_quality_gate_summary.json`
    - `build/reports/unity_miq_compression_probe.json`
- Added Unity/Native parity gate script:
  - `tools/miq_parity_gate.ps1`
  - outputs:
    - `build/reports/miq_parity_gate_summary.txt`
    - `build/reports/miq_parity_gate_summary.json`
    - `build/reports/unity_miq_parity_probe.json`
...
- Updated baseline gate runner:
  ...
  - new flags:
    - `-EnableMiqCompressionQuality`
    - `-EnableMiqParity`
- Updated release readiness entry:
  - `tools/release_readiness_gate.ps1`
  - passes through the same MIQ quality/parity toggles into baseline execution
  - summary artifact list expanded with new MIQ gate reports
- Updated dashboard:
  - `tools/release_gate_dashboard.ps1`
  - new rows:
    - `MIQ Compression Quality`
    - `MIQ Unity/Native Parity`
  - release candidate criteria now additionally requires:
    - `Unity MIQ Validate = PASS`
    - `MIQ Compression Quality = PASS`
    - `MIQ Unity/Native Parity = PASS`

### C) CI workflow enforcement path

- Updated `NativeAnimiq/.github/workflows/unity-miq-compat.yml`:
  - executes:
    1. `unity_miq_validate.ps1`
    2. `miq_compression_quality_gate.ps1`
    3. native `avatar_tool` build
    4. `miq_parity_gate.ps1`
