# VRM to MIQ v5 Exporter Upgrade (2026-03-06)

## Scope

This report documents the `tools/vrm_to_miq.cpp` uplift from a minimal mesh/material exporter into a v5-capable runtime-ready exporter.

In scope:

- MIQ file version uplift (`v1 -> v5`)
...
- Changed exporter output to `MIQ v5`.
...
Previously, `vrm_to_miq` wrote only:
...
- Emits section flag `0x0001` and size envelope format compatible with current MIQ loader behavior.
...
`vrm_to_miq [--strict] [--no-compress] [--diag-json <path>] <input.vrm> <output.miq>`
...
- Build:
  ...
  - Result: PASS
- Sample conversion:
  - `vrm_to_miq --diag-json NativeAnimiq/build/reports/vrm_to_miq_diag.json "sample/Kikyo_FT Variant.vrm" "build/kikyo_test.miq"`
  - Result: PASS
  - Summary:
    - `MIQ(v5)` written
    - `Sections=208`
    ...
    - `BlendShapes=16`
- Load validation:
  - `avatar_tool "build/kikyo_test.miq"`
  - Result: PASS
  - Key output:
    - `Format=MIQ`
    - `Compat=full`
...
- Strict mode sample:
  - `vrm_to_miq --strict "sample/Kikyo_FT Variant.vrm" "build/kikyo_test_strict.miq"`
  - Result: PASS
...
1. Add a dedicated `vrm_to_miq` parity gate script validating section counts/flags across fixed sample sets.
