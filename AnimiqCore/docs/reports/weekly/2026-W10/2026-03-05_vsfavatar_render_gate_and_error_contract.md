...
```powershell
cmake --build NativeAnimiq\build --config Release --target vsfavatar_sidecar nativecore avatar_tool
dotnet build NativeAnimiq\host\HostCore\HostCore.csproj -c Release
dotnet build NativeAnimiq\host\WpfHost\WpfHost.csproj -c Release --no-restore -nr:false
