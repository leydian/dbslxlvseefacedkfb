...
.\NativeAnimiq\.venv\Scripts\python.exe --version
.\NativeAnimiq\.venv\Scripts\python.exe -c "import mediapipe, cv2; print('venv import ok')"
```
...
```powershell
[Environment]::GetEnvironmentVariable("ANIMIQ_MEDIAPIPE_PYTHON", "User")
```
...
- If workspace path changes, `ANIMIQ_MEDIAPIPE_PYTHON` must be updated to the new absolute path.
