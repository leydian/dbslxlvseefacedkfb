...
- WPF UI/동작 추가:
  ...
  - 썸네일 경로는 `%LOCALAPPDATA%/AnimiqHost/thumbnails/<sha256>.png`를 사용한다.
  - 경로:
...
- 실행 커맨드:
  - `cmake -S NativeAnimiq -B NativeAnimiq/build-thumb -DANIMIQ_BUILD_CONSOLE=ON`
  - `cmake --build NativeAnimiq/build-thumb --config Release --target nativecore`
