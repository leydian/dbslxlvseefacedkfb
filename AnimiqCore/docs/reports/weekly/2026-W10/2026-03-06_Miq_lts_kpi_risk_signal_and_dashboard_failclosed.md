# MIQ LTS KPI Risk Signal + Dashboard Fail-Closed Update (2026-03-06)

## Summary

This update hardens the Unity LTS support operation model for MIQ after
...
### 2) Dashboard fail-closed behavior for Unity MIQ
...
- Unity MIQ final status evaluation now uses explicit state branching:
...
- reads `build/reports/unity_miq_lts_kpi_summary.json` when available
- emits risk fields in dashboard JSON:
  - `unity_miq_lts_recent_risk`
  - `unity_miq_lts_recent_risk_lines`
- emits risk summary lines in dashboard text output:
  - `UnityMiqLtsRecentRisk`
  - `UnityMiqLtsRecentRiskLines`
...
- `CHANGELOG.md`
- `docs/reports/weekly/2026-W10/2026-03-06_miq_unity_lts_matrix_gate_policy.md`
...
[scriptblock]::Create((Get-Content -Raw .\tools\unity_miq_lts_gate.ps1))
