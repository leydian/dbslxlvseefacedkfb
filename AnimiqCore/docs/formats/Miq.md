# MIQ Format (Draft v1/v2/v3/v4/v5)

MIQ is a vxa2-derived container focused on runtime-ready mesh/material transport.
...
- If missing/unknown on MIQ, runtime uses `dx_row_major`.
- Auto-detect is disabled by default and can be enabled only for diagnostics with
  `ANIMIQ_MIQ_ENABLE_CONVENTION_AUTODETECT=1`.
- Legacy MIQ without `skinSpaceBasis=mesh_local` may use compatibility auto-detect
...
If section payload crosses file boundary, loader returns `MIQ_SECTION_TRUNCATED`.
...
- `0x0001`: payload is LZ4-compressed envelope (`v5+`)
  - payload layout:
    ...
    - `compressed_lz4[...]`
- unknown flag bits are reserved; loader records `MIQ_SECTION_FLAGS_NONZERO` warning.
...
- Recognized extension: `.miq`
- Unknown section policy:
  - `Warn` (default): unknown sections are skipped and warning `MIQ_UNKNOWN_SECTION` is emitted.
  - `Ignore`: unknown sections are skipped without warning.
  - `Fail`: load terminates with primary error `MIQ_UNKNOWN_SECTION_NOT_ALLOWED`.
- Loader diagnostics include both raw `warnings[]` and normalized `warning_codes[]`.
- Missing referenced payloads produce `MIQ_ASSET_MISSING` and `Compat: partial`.
- Warning-code contract (tooling/host):
  ...
  - `MIQ_*|XAV3_*|XAV4_*`: severity=`warn`, category=`render`
...
This update resolves a set of MIQ runtime parity regressions observed with VRM-derived avatars.

- Material alpha parity:
  - Root cause: runtime alpha heuristics could override MIQ payload `alpha_mode` (example: `OPAQUE -> MASK`).
  - Fix: for MIQ, runtime now trusts payload `alpha_mode` when present and does not re-promote via heuristic path.
  - Validation target:
    - `MaterialParityMismatchCount = 0`
    - `MaterialParityLastMismatch = none`

- Typed material schema drift:
  - Root cause: `vrm_to_miq` could emit typed payload body as legacy layout while manifest advertised `typed-v3`.
  - Fix:
    - exporter normalizes typed schema version to `>= v3` for MIQ typed sections.
  ...
  - Validation target:
    - no `MIQ_MATERIAL_TYPED_SCHEMA_CONFLICT` warning for newly exported assets.

- Preview facing direction (front/back):
  - Root cause: preview yaw policy treated all MIQ as fixed `0deg`, which can show back-facing view for VRM-derived exports.
  - Fix:
    - MIQ loader stores manifest `sourceExt`.
  ...
  - Validation target (`LastRenderPassSummary`):
    ...
    - `preview_yaw_reason=miq_vrm_source_180`
...
Operational note:
- Runtime DLL replacement alone does not rewrite existing `.miq` payloads.
- To pick up typed schema normalization fixes, re-export VRM to MIQ with updated `vrm_to_miq`.

## 2026-03-06 Follow-up Stabilization (Runtime)

Additional runtime hardening was applied after parity fixes to address legacy/edge MIQ behavior:

- VRM-derived MIQ static skinning policy:
...
- Legacy skinning convention fallback:
  - For legacy MIQ without `skinSpaceBasis`, runtime convention hint prefers VRM-origin defaults
...
- Outlier mesh handling consistency:
  ...
  - Draw exclusion policy is runtime-configurable via `ANIMIQ_MIQ_OUTLIER_DRAW_POLICY`:
  ...
  - Related warning codes:
    - `MIQ_BOUNDS_OUTLIER_DRAW_SKIPPED`
    - `MIQ_DETACHED_MESH_OUTLIER_SKIPPED`
    - `MIQ_DETACHED_CLUSTER_SKIPPED`
    - `MIQ_EXTREME_DETACHED_CLUSTER_SKIPPED`
    - `MIQ_VRM_ORIGIN_DETACHED_CLUSTER_SKIPPED`

- AutoFit bust framing stability:
  - `focus_y` in `AutoFitBust` now blends bounds-derived and robust-center-derived anchors for MIQ,
