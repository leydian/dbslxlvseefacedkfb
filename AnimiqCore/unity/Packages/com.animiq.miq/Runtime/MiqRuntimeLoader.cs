...
{
    public static class MiqRuntimeLoader
    {
        ...
        /// <summary>
        /// Loads an MIQ avatar payload from disk and throws on failure.
        /// </summary>
        public static MiqAvatarPayload Load(string path)
        {
        ...
        /// <summary>
        /// Loads an MIQ avatar payload from disk without throwing.
        /// </summary>
        public static bool TryLoad(string path, out MiqAvatarPayload payload, out MiqLoadDiagnostics diagnostics)
        {
            return TryLoad(path, out payload, out diagnostics, new MiqLoadOptions());
        }

        /// <summary>
        /// Loads an MIQ avatar payload from disk using explicit loader options.
        /// </summary>
        public static bool TryLoad(
            string path,
            out MiqAvatarPayload payload,
            out MiqLoadDiagnostics diagnostics,
            MiqLoadOptions options)
        {
            payload = new MiqAvatarPayload();
            diagnostics = new MiqLoadDiagnostics();
            options ??= new MiqLoadOptions();
            ...
            {
                return Fail(diagnostics, MiqLoadErrorCode.MagicMismatch, "MIQ header is truncated.");
            }
            if (!string.Equals(magic, "MIQ", StringComparison.Ordinal))
            {
                return Fail(diagnostics, MiqLoadErrorCode.MagicMismatch, "MIQ magic mismatch.");
            }
            diagnostics.ParserStage = "parse";

            if (!TryReadUInt16(bytes, ref cursor, out var version))
            {
                return Fail(diagnostics, MiqLoadErrorCode.UnsupportedVersion, "MIQ version field is truncated.");
            }
            if (version != 1 && version != 2 && version != 3 && version != 4 && version != 5)
            {
                return Fail(diagnostics, MiqLoadErrorCode.UnsupportedVersion, $"Unsupported MIQ version: {version}");
            }
            diagnostics.SourceFormatVersion = version;

            if (!TryReadUInt32(bytes, ref cursor, out var manifestSize))
            {
                return Fail(diagnostics, MiqLoadErrorCode.ManifestTruncated, "Manifest size field is truncated.");
            }
            if (manifestSize > (uint)(bytes.Length - cursor))
            {
                return Fail(diagnostics, MiqLoadErrorCode.ManifestTruncated, "Manifest bytes are out of file range.");
            }
            if (!TryReadUtf8(bytes, ref cursor, (int)manifestSize, out var manifestJson))
            {
                return Fail(diagnostics, MiqLoadErrorCode.ManifestTruncated, "Manifest payload is truncated.");
            }

            MiqManifest manifest;
            try
            {
                manifest = JsonUtility.FromJson<MiqManifest>(manifestJson) ?? new MiqManifest();
            }
            catch (Exception ex)
            {
                return Fail(diagnostics, MiqLoadErrorCode.ManifestInvalid, $"Manifest JSON parse failed: {ex.Message}");
            }
                ...
                manifest.textureRefs.Count == 0)
            {
                return Fail(
                    diagnostics,
                    MiqLoadErrorCode.MissingRequiredManifestKeys,
                    "Manifest requires avatarId/meshRefs/materialRefs/textureRefs.");
            }
            diagnostics.ParserStage = "resolve";

            var materialsByName = new Dictionary<string, MiqMaterialPayload>(StringComparer.OrdinalIgnoreCase);
            while (cursor < bytes.Length)
            {
                ...
                if ((bytes.Length - cursor) < 8)
                {
                    return Fail(
                        diagnostics,
                        MiqLoadErrorCode.SectionHeaderTruncated,
                ...
                if (sectionSize > (uint)(bytes.Length - cursor))
                {
                    return Fail(
                        diagnostics,
                        MiqLoadErrorCode.SectionTruncated,
                ...
                if ((sectionFlags & SectionFlagPayloadCompressedLz4) != 0)
                {
                    if (version < 5)
                    {
                        return Fail(
                            diagnostics,
                            MiqLoadErrorCode.SectionSchemaInvalid,
                            $"Compressed section is not supported for MIQ version {version}: type=0x{sectionType:X4}.");
                    }
                            ...
                            out var compressionError))
                    {
                        return Fail(
                            diagnostics,
                            MiqLoadErrorCode.CompressionDecodeFailed,
                ...
                if (unknownFlags != 0)
                {
                    if (!AddWarningOrFail(
                            diagnostics,
                            options,
                            $"MIQ_SECTION_FLAGS_NONZERO: type=0x{sectionType:X4}, flags={unknownFlags}"))
                    {
                        return false;
                    }
                }
            ...
            out string error)
        {
            ...
            if (!MiqLz4Codec.TryDecompress(compressed, expectedLength, out decoded))
            {
        ...
        private static bool TryParseSection(
            ...
            int sectionLength,
            int formatVersion,
            MiqAvatarPayload payload,
            Dictionary<string, MiqMaterialPayload> materialsByName,
            MiqLoadDiagnostics diagnostics,
            MiqLoadOptions options)
        {
            switch (sectionType)
            {
        ...
        private static bool HandleUnknownSection(
            ...
            ushort sectionType)
        {
            var warning = $"MIQ_UNKNOWN_SECTION: 0x{sectionType:X4}";
            var policy = options?.UnknownSectionPolicy ?? MiqUnknownSectionPolicy.Warn;
            switch (policy)
            {
                case MiqUnknownSectionPolicy.Ignore:
                    return true;
                case MiqUnknownSectionPolicy.Fail:
                    return Fail(
                        diagnostics,
                        MiqLoadErrorCode.UnknownSectionNotAllowed,
                        $"Unknown MIQ section is not allowed by policy: 0x{sectionType:X4}");
                case MiqUnknownSectionPolicy.Warn:
                default:
            ...
            MiqLoadOptions options)
        {
            ...
            {
                return Fail(diagnostics, MiqLoadErrorCode.SectionSchemaInvalid, "Invalid MIQ mesh section.");
            }
            ...
            for (var i = 0; i < indexCount; i++)
            {
                ...
                {
                    return Fail(diagnostics, MiqLoadErrorCode.SectionSchemaInvalid, "Invalid MIQ mesh index array.");
                }
            }

            if (ms.Position != ms.Length)
            {
                return AddWarningOrFail(diagnostics, options, $"MIQ_MESH_TRAILING_BYTES: mesh={name}");
            }
            ...
            MiqLoadOptions options)
        {
            ...
            {
                return Fail(diagnostics, MiqLoadErrorCode.SectionSchemaInvalid, "Invalid MIQ texture section.");
            }

            if (ms.Position != ms.Length)
            {
                return AddWarningOrFail(diagnostics, options, $"MIQ_TEXTURE_TRAILING_BYTES: texture={name}");
            }
            ...
            MiqLoadOptions options)
        {
            ...
            {
                return Fail(diagnostics, MiqLoadErrorCode.SectionSchemaInvalid, "Invalid MIQ material section.");
            }

            var material = new MiqMaterialPayload
            {
                Name = name,
            ...
            if (!parsedWithVariant)
            {
                    ...
                    !TryReadByte(br, out var legacyDoubleSided))
                {
                    return Fail(
                        diagnostics,
                        MiqLoadErrorCode.SectionSchemaInvalid,
                        $"Invalid MIQ material section payload for '{name}'.");
                }
                ...
                if (ms.Position != ms.Length)
                {
                    return Fail(
                        diagnostics,
                        MiqLoadErrorCode.SectionSchemaInvalid,
                        $"Invalid trailing bytes in MIQ material section '{name}'.");
                }

                if (!AddWarningOrFail(diagnostics, options, $"MIQ_MATERIAL_LEGACY_VARIANT: material={name}"))
                {
                    return false;
                }
            }
            ...
            MiqLoadOptions options)
        {
            ...
            {
                return Fail(diagnostics, MiqLoadErrorCode.SectionSchemaInvalid, "Invalid MIQ material params section.");
            }

            if (ms.Position != ms.Length)
            {
                return AddWarningOrFail(diagnostics, options, $"MIQ_MATERIAL_PARAMS_TRAILING_BYTES: material={name}");
            }

            if (!materialsByName.TryGetValue(name, out var material))
            {
                material = new MiqMaterialPayload { Name = name };
                materialsByName[name] = material;
            }
            ...
            bool preferTypedV3)
        {
            ...
            {
                return Fail(diagnostics, MiqLoadErrorCode.SectionSchemaInvalid, "Invalid MIQ material typed params section.");
            }
            ...
            if (!parsed)
            {
                ...
                if (!TryParseTypedMaterialBody(br, floatCount, material) || ms.Position != ms.Length)
                {
                    ...
                    if (!parsed)
                    {
                        return AddWarningOrFail(diagnostics, options, $"MIQ_MATERIAL_TYPED_SCHEMA_INVALID: material={name}");
                    }
                }
            }
            ...
            if (!string.Equals(material.ShaderFamily, "legacy", StringComparison.OrdinalIgnoreCase))
            {
                ...
                {
                    if (!AddWarningOrFail(diagnostics, options, $"MIQ_MATERIAL_TYPED_MISSING_REQUIRED_PARAM: material={name}, id=_BaseColor"))
                    {
                        return false;
                    }
                }
            ...
            if (!IsSupportedShaderFamily(material.ShaderFamily))
            {
                if (!AddWarningOrFail(
                        diagnostics,
                        options,
                        $"MIQ_MATERIAL_TYPED_UNSUPPORTED_SHADER_FAMILY: material={name}, family={material.ShaderFamily}"))
                {
                    return false;
                }
            }
        ...
        private static bool TryParseTypedMaterialBody(BinaryReader br, ushort floatCount, MiqMaterialPayload material)
        {
            for (var i = 0; i < floatCount; i++)
            {
                ...
                material.TypedFloatParams.Add(new MiqTypedFloatParam { Id = id, Value = value });
            }
            ...
            for (var i = 0; i < colorCount; i++)
            {
                ...
                material.TypedColorParams.Add(new MiqTypedColorParam { Id = id, R = r, G = g, B = b, A = a });
            }
            ...
            for (var i = 0; i < textureCount; i++)
            {
                ...
                material.TypedTextureParams.Add(new MiqTypedTextureParam
                {
                    Slot = slot,
            ...
            MiqLoadOptions options)
        {
            ...
            {
                return Fail(diagnostics, MiqLoadErrorCode.SectionSchemaInvalid, "Invalid MIQ skin section.");
            }
            ...
            for (var i = 0; i < boneCount; i++)
            {
                ...
                {
                    return Fail(diagnostics, MiqLoadErrorCode.SectionSchemaInvalid, "Invalid MIQ skin bone indices.");
                }
            }

            if (!TryReadUInt32(br, out var bindPoseCount))
            {
                return Fail(diagnostics, MiqLoadErrorCode.SectionSchemaInvalid, "Invalid MIQ skin bindpose count.");
            }
            ...
            for (var i = 0; i < bindPoseCount; i++)
            {
                ...
                {
                    return Fail(diagnostics, MiqLoadErrorCode.SectionSchemaInvalid, "Invalid MIQ skin bindpose payload.");
                }
            }

            if (!TryReadBytesWithUInt32Length(br, out var skinWeightBlob))
            {
                return Fail(diagnostics, MiqLoadErrorCode.SectionSchemaInvalid, "Invalid MIQ skin weight payload.");
            }

            if (ms.Position != ms.Length)
            {
                return AddWarningOrFail(diagnostics, options, $"MIQ_SKIN_TRAILING_BYTES: mesh={meshName}");
            }

            payload.Skins.Add(new MiqSkinPayload
            {
                MeshName = meshName,
            ...
            MiqLoadOptions options)
        {
            ...
            {
                return Fail(diagnostics, MiqLoadErrorCode.SectionSchemaInvalid, "Invalid MIQ blendshape section.");
            }

            var blendPayload = new MiqBlendShapePayload { MeshName = meshName };
            for (var i = 0; i < frameCount; i++)
            {
                ...
                {
                    return Fail(diagnostics, MiqLoadErrorCode.SectionSchemaInvalid, "Invalid MIQ blendshape frame payload.");
                }

                blendPayload.Frames.Add(new MiqBlendShapeFramePayload
                {
                    Name = frameName,
                    Weight = weight,
            ...
            if (ms.Position != ms.Length)
            {
                return AddWarningOrFail(diagnostics, options, $"MIQ_BLENDSHAPE_TRAILING_BYTES: mesh={meshName}");
            }
            ...
            MiqLoadOptions options)
        {
            ...
            {
                return Fail(diagnostics, MiqLoadErrorCode.SectionSchemaInvalid, "Invalid MIQ skeleton pose section.");
            }
            ...
            for (var i = 0; i < matrixValueCount; i++)
            {
                ...
                {
                    return Fail(diagnostics, MiqLoadErrorCode.SectionSchemaInvalid, "Invalid MIQ skeleton matrix payload.");
                }
            }

            if (ms.Position != ms.Length)
            {
                return AddWarningOrFail(diagnostics, options, $"MIQ_SKELETON_TRAILING_BYTES: mesh={meshName}");
            }

            payload.Skeletons.Add(new MiqSkeletonPayload
            {
                MeshName = meshName,
            ...
            MiqLoadOptions options)
        {
            ...
            if (!TryReadSizedString(br, out var meshName) || !TryReadUInt32(br, out var boneCount))
            {
                return Fail(diagnostics, MiqLoadErrorCode.SectionSchemaInvalid, "Invalid MIQ skeleton rig section.");
            }

            var rig = new MiqSkeletonRigPayload
            {
                MeshName = meshName
            };
            ...
            for (var i = 0; i < boneCount; i++)
            {
                ...
                {
                    return Fail(diagnostics, MiqLoadErrorCode.SectionSchemaInvalid, "Invalid MIQ skeleton rig bone entry.");
                }
                ...
                for (var j = 0; j < matrixValueCount; j++)
                {
                    ...
                    {
                        return Fail(diagnostics, MiqLoadErrorCode.SectionSchemaInvalid, "Invalid MIQ skeleton rig matrix payload.");
                    }
                }

                rig.Bones.Add(new MiqRigBonePayload
                {
                    Name = boneName,
            ...
            MiqLoadOptions options)
        {
            ...
            {
                return Fail(diagnostics, MiqLoadErrorCode.SectionSchemaInvalid, "Invalid MIQ physics collider section.");
            }
            ...
            if (ms.Position != ms.Length)
            {
                return AddWarningOrFail(
                    diagnostics,
                    options,
                    $"MIQ_PHYSICS_SCHEMA_INVALID: collider={name}, issue=trailing-bytes");
            }
            ...
            payload.PhysicsColliders.Add(new MiqPhysicsColliderPayload
            {
                Name = name,
                BonePath = bonePath,
                Shape = Enum.IsDefined(typeof(MiqPhysicsColliderShape), (int)shapeRaw)
                    ? (MiqPhysicsColliderShape)shapeRaw
                    : MiqPhysicsColliderShape.Unknown,
                Radius = radius,
                Height = height,
            ...
            MiqLoadOptions options)
        {
            ...
            {
                return Fail(diagnostics, MiqLoadErrorCode.SectionSchemaInvalid, "Invalid MIQ springbone section.");
            }
            ...
            if (ms.Position != ms.Length)
            {
                return AddWarningOrFail(
                    diagnostics,
                    options,
                    $"MIQ_PHYSICS_SCHEMA_INVALID: springBone={name}, issue=trailing-bytes");
            }
            ...
            MiqLoadOptions options)
        {
            ...
            if (!TryReadSizedString(br, out var name) ||
                ...
                !TryReadByte(br, out var enabledByte))
            {
                return Fail(diagnostics, MiqLoadErrorCode.SectionSchemaInvalid, "Invalid MIQ physbone section.");
            }
            ...
            if (ms.Position != ms.Length)
            {
                return AddWarningOrFail(
                    diagnostics,
                    options,
                    $"MIQ_PHYSICS_SCHEMA_INVALID: physBone={name}, issue=trailing-bytes");
            }
        ...
        private static void NormalizeManifest(MiqManifest manifest)
        {
            ...
            MiqLoadOptions options)
        {
            ...
            foreach (var material in payload.Materials)
            {
                ...
                {
                    var policy = options?.ShaderPolicy ?? MiqShaderPolicy.WarnFallback;
                    if (policy == MiqShaderPolicy.Fail)
                    {
                        diagnostics.CriticalParityViolation = true;
                        return Fail(
                            diagnostics,
                            MiqLoadErrorCode.ParityContractViolation,
                    ...
                    if (!AddWarningOrFail(
                            diagnostics,
                            options,
                            $"MIQ_SHADER_FAMILY_FALLBACK: material={material.Name}, from={fromFamily}, to={material.ShaderFamily}"))
                    {
                        return false;
                    }
                }
                ...
                if (!hasTyped || !string.Equals(material.MaterialParamEncoding, "typed-v4", StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    if (!material.TypedColorParams.Exists(p => string.Equals(p.Id, "_BaseColor", StringComparison.Ordinal)))
                    {
                        material.TypedColorParams.Add(new MiqTypedColorParam
                        {
                            Id = "_BaseColor",
                            R = 1.0f,
                            G = 1.0f,
            ...
            int formatVersion)
        {
            ...
            foreach (var meshRef in payload.Manifest.meshRefs)
            {
                ...
                {
                    missingMeshRef = true;
                    if (!AddWarningOrFail(diagnostics, options, $"MIQ_ASSET_MISSING: meshRef='{meshRef}'"))
                    {
                        return false;
                    }
                }
            ...
            foreach (var textureRef in payload.Manifest.textureRefs)
            {
                if (!textureNameSet.Contains(NormalizeRefKey(textureRef)))
                {
                    ...
                    if (!AddWarningOrFail(diagnostics, options, $"MIQ_ASSET_MISSING: textureRef='{textureRef}'"))
                    {
                        return false;
                    }
                }
            ...
            foreach (var material in payload.Materials)
            {
                ...
                if (fallbackReasons.Count > 0)
                {
                    if (!AddWarningOrFail(
                            diagnostics,
                            options,
                            $"MIQ_MATERIAL_FALLBACK_APPLIED: material={material.Name}, reason={string.Join("|", fallbackReasons)}"))
                    {
                        return false;
                    }
                }
            ...
            if (payload.Manifest.materialRefs.Count != 0 && payload.Materials.Count == 0)
            {
                if (!AddWarningOrFail(
                        diagnostics,
                        options,
                        "MIQ_ASSET_MISSING: materialRefs exist but no material payloads were parsed."))
                {
                    return false;
                }
                ...
                foreach (var springBone in payload.SpringBones)
                {
                    foreach (var colliderRef in springBone.ColliderRefs)
                    {
                        ...
                        if (!AddWarningOrFail(
                                diagnostics,
                                options,
                                $"MIQ_PHYSICS_REF_MISSING: springBone={springBone.Name}, collider={colliderRef}"))
                        {
                            return false;
                        }
                    }
                ...
                foreach (var physBone in payload.PhysBones)
                {
                    foreach (var colliderRef in physBone.ColliderRefs)
                    {
                        ...
                        if (!AddWarningOrFail(
                                diagnostics,
                                options,
                                $"MIQ_PHYSICS_REF_MISSING: physBone={physBone.Name}, collider={colliderRef}"))
                        {
                            return false;
                        }
                    }
        ...
        private static bool HasTypedMaterialPayload(MiqMaterialPayload material)
        {
            if (material == null)
            {
                return false;
        ...
        private static bool AddWarningOrFail(MiqLoadDiagnostics diagnostics, MiqLoadOptions options, string warning)
        {
        ...
        private static void AddSoftWarning(MiqLoadDiagnostics diagnostics, string warning)
        {
        ...
        private static void AddWarningCode(MiqLoadDiagnostics diagnostics, string warning)
        {
            if (diagnostics == null || string.IsNullOrWhiteSpace(warning))
            {
                return;
        ...
        private static bool Fail(MiqLoadDiagnostics diagnostics, MiqLoadErrorCode errorCode, string errorMessage)
        {
        ...
        private static Exception BuildLoadException(string path, MiqLoadDiagnostics diagnostics)
        {
            ...
            return new InvalidDataException(
                $"Failed to load MIQ '{path}' at stage '{diagnostics.ParserStage}' with '{diagnostics.ErrorCode}': {message}");
        }
