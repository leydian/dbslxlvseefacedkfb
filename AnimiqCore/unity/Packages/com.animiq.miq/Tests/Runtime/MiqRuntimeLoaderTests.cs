...
namespace Animiq.Miq.Runtime.Tests
{
    public sealed class MiqRuntimeLoaderTests
    {
        [Test]
        public void TryLoad_ValidPayload_ReturnsRuntimeReady()
        {
            var path = WriteTempFile(BuildValidMiqBytes());
            try
            {
                var ok = MiqRuntimeLoader.TryLoad(path, out var payload, out var diagnostics);
        ...
        public void TryLoad_MagicMismatch_Fails()
        {
            ...
            try
            {
                ...
                Assert.That(diagnostics.ErrorCode, Is.EqualTo(MiqLoadErrorCode.MagicMismatch));
            }
            finally
            {
                File.Delete(path);
        ...
        public void TryLoad_UnsupportedVersion_Fails()
        {
            ...
            try
            {
                var ok = MiqRuntimeLoader.TryLoad(path, out _, out var diagnostics);
                Assert.That(ok, Is.False);
                Assert.That(diagnostics.ErrorCode, Is.EqualTo(MiqLoadErrorCode.UnsupportedVersion));
            }
            finally
            {
                File.Delete(path);
        ...
        {
            var path = WriteTempFile(BuildValidMiqBytes(addTypedMaterialSection: true));
            try
            {
                var ok = MiqRuntimeLoader.TryLoad(path, out var payload, out var diagnostics);
                Assert.That(ok, Is.True);
        ...
        {
            var path = WriteTempFile(BuildValidMiqBytes(addTypedMaterialSection: true, typedIncludeAdvancedEntries: true));
            try
            {
                var ok = MiqRuntimeLoader.TryLoad(path, out var payload, out var diagnostics);
                Assert.That(ok, Is.True);
        ...
        public void TryLoad_TypedMaterialParams_AdvancedPoiyomiEntries_Parses()
        {
            ...
            try
            {
                ...
                Assert.That(ok, Is.True);
                Assert.That(diagnostics.ErrorCode, Is.EqualTo(MiqLoadErrorCode.None));
                Assert.That(diagnostics.WarningCodes, Does.Not.Contain("MIQ_MATERIAL_TYPED_UNSUPPORTED_SHADER_FAMILY"));
                var material = payload.Materials[0];
        ...
        {
            var path = WriteTempFile(BuildValidMiqBytes(addTypedMaterialSection: true, typedSchemaVersion: 2));
            try
            {
                var ok = MiqRuntimeLoader.TryLoad(path, out var payload, out var diagnostics);
                Assert.That(ok, Is.True);
        ...
        public void TryLoad_TypedMaterialParamsV2_MissingBaseColor_Strict_Fails()
        {
            ...
            try
            {
                var ok = MiqRuntimeLoader.TryLoad(
                    ...
                    out _,
                    out var diagnostics,
                    new MiqLoadOptions { StrictValidation = true });
                Assert.That(ok, Is.False);
                Assert.That(diagnostics.ErrorCode, Is.EqualTo(MiqLoadErrorCode.StrictValidationFailed));
                Assert.That(diagnostics.Warnings.Exists(w => w.Contains("MIQ_MATERIAL_TYPED_MISSING_REQUIRED_PARAM")), Is.True);
            }
            finally
            {
                File.Delete(path);
        ...
        public void TryLoad_TypedMaterialParamsV2_UnsupportedShaderFamily_FailPolicy_Fails()
        {
            ...
            try
            {
                var ok = MiqRuntimeLoader.TryLoad(
                    ...
                    out _,
                    out var diagnostics,
                    new MiqLoadOptions { ShaderPolicy = MiqShaderPolicy.Fail });
                Assert.That(ok, Is.False);
                Assert.That(diagnostics.ErrorCode, Is.EqualTo(MiqLoadErrorCode.ParityContractViolation));
                Assert.That(diagnostics.WarningCodes, Does.Contain("MIQ_MATERIAL_TYPED_UNSUPPORTED_SHADER_FAMILY"));
            }
            finally
            {
                File.Delete(path);
        ...
        public void TryLoad_TypedMaterialParamsV2_InvalidTrailingData_Strict_Fails()
        {
            ...
            try
            {
                ...
                Assert.That(ok, Is.False);
                Assert.That(diagnostics.ErrorCode, Is.EqualTo(MiqLoadErrorCode.StrictValidationFailed));
                Assert.That(diagnostics.WarningCodes, Does.Contain("MIQ_MATERIAL_TYPED_SCHEMA_INVALID"));
            }
            finally
            {
                File.Delete(path);
        ...
        {
            var path = WriteTempFile(BuildValidMiqBytes(addTypedMaterialSection: true, unresolvedTypedTextureRef: true));
            try
            {
                var ok = MiqRuntimeLoader.TryLoad(path, out _, out var diagnostics);
                Assert.That(ok, Is.True);
                Assert.That(
                    diagnostics.Warnings.Exists(w => w.Contains("MIQ_MATERIAL_TYPED_TEXTURE_UNRESOLVED")),
                    Is.True);
                Assert.That(diagnostics.WarningCodes, Does.Contain("MIQ_MATERIAL_TYPED_TEXTURE_UNRESOLVED"));
            }
            finally
            {
                File.Delete(path);
        ...
        public void TryLoad_TypedMaterialTextureRef_NormalizedMatch_DoesNotWarn()
        {
            ...
            try
            {
                ...
                Assert.That(
                    diagnostics.Warnings.Exists(w => w.Contains("MIQ_MATERIAL_TYPED_TEXTURE_UNRESOLVED")),
                    Is.False);
                Assert.That(diagnostics.WarningCodes, Does.Not.Contain("MIQ_MATERIAL_TYPED_TEXTURE_UNRESOLVED"));
            }
            finally
            {
                File.Delete(path);
        ...
        public void TryLoad_TypedMaterialSupportedShaderFamily_DoesNotWarnUnsupported()
        {
            ...
            try
            {
                ...
                Assert.That(ok, Is.True);
                Assert.That(
                    diagnostics.Warnings.Exists(w => w.Contains("MIQ_MATERIAL_TYPED_UNSUPPORTED_SHADER_FAMILY")),
                    Is.False);
                Assert.That(diagnostics.WarningCodes, Does.Not.Contain("MIQ_MATERIAL_TYPED_UNSUPPORTED_SHADER_FAMILY"));
            }
            finally
        ...
        public void TryLoad_TypedMaterialUnsupportedShaderFamily_DefaultPolicy_WarnsAndFallsBack()
        {
            ...
            try
            {
                ...
                Assert.That(diagnostics.ErrorCode, Is.EqualTo(MiqLoadErrorCode.None));
                Assert.That(payload.Materials[0].ShaderFamily, Is.EqualTo("standard"));
                Assert.That(diagnostics.WarningCodes, Does.Contain("MIQ_SHADER_FAMILY_FALLBACK"));
        ...
        {
            var path = WriteTempFile(BuildValidMiqBytes(addSkinSection: true));
            try
            {
                var ok = MiqRuntimeLoader.TryLoad(path, out _, out var diagnostics);
                Assert.That(ok, Is.True);
        ...
        {
            var path = WriteTempFile(BuildValidMiqBytes(addSkinSection: true, formatVersion: 4));
            try
            {
                var ok = MiqRuntimeLoader.TryLoad(path, out _, out var diagnostics);
                Assert.That(ok, Is.True);
        ...
        {
            var path = WriteTempFile(BuildValidMiqBytes(addSkinSection: true, addRigSection: true, formatVersion: 4));
            try
            {
                var ok = MiqRuntimeLoader.TryLoad(path, out var payload, out var diagnostics);
                Assert.That(ok, Is.True);
        ...
        {
            var path = WriteTempFile(BuildValidMiqBytes(addSkinSection: true, addRigSection: true, formatVersion: 4, rigDuplicateBoneName: true));
            try
            {
                var ok = MiqRuntimeLoader.TryLoad(path, out _, out var diagnostics);
                Assert.That(ok, Is.True);
        ...
        {
            var path = WriteTempFile(BuildValidMiqBytes(addSkinSection: true, addRigSection: true, formatVersion: 4, rigCycle: true));
            try
            {
                var ok = MiqRuntimeLoader.TryLoad(
                    path,
        ...
        public void TryLoad_ManifestTruncated_Fails()
        {
            ...
            try
            {
                var ok = MiqRuntimeLoader.TryLoad(path, out _, out var diagnostics);
                Assert.That(ok, Is.False);
                Assert.That(diagnostics.ErrorCode, Is.EqualTo(MiqLoadErrorCode.ManifestTruncated));
            }
            finally
            {
                File.Delete(path);
        ...
        {
            var path = WriteTempFile(BuildValidMiqBytes(formatVersion: 5, compressMeshSection: true));
            try
            {
                var ok = MiqRuntimeLoader.TryLoad(path, out var payload, out var diagnostics);
                Assert.That(ok, Is.True);
        ...
        public void TryLoad_CompressedMeshSectionCorrupt_FailsDecode()
        {
            ...
            try
            {
                var ok = MiqRuntimeLoader.TryLoad(path, out _, out var diagnostics);
                Assert.That(ok, Is.False);
                Assert.That(diagnostics.ErrorCode, Is.EqualTo(MiqLoadErrorCode.CompressionDecodeFailed));
            }
            finally
            {
                File.Delete(path);
        ...
        public void TryLoad_CompressedMeshSectionV4_FailsSchemaInvalid()
        {
            ...
            try
            {
                var ok = MiqRuntimeLoader.TryLoad(path, out _, out var diagnostics);
                Assert.That(ok, Is.False);
                Assert.That(diagnostics.ErrorCode, Is.EqualTo(MiqLoadErrorCode.SectionSchemaInvalid));
            }
            finally
            {
                File.Delete(path);
        ...
        public void TryLoad_SectionTruncated_Fails()
        {
            ...
            try
            {
                var ok = MiqRuntimeLoader.TryLoad(path, out _, out var diagnostics);
                Assert.That(ok, Is.False);
                Assert.That(diagnostics.ErrorCode, Is.EqualTo(MiqLoadErrorCode.SectionTruncated));
            }
            finally
            {
                File.Delete(path);
        ...
        {
            var path = WriteTempFile(BuildValidMiqBytes(legacyMaterialFormat: true));
            try
            {
                var ok = MiqRuntimeLoader.TryLoad(path, out var payload, out var diagnostics);
                Assert.That(ok, Is.True);
        ...
        {
            var path = WriteTempFile(BuildValidMiqBytes(addUnknownSection: true));
            try
            {
                var ok = MiqRuntimeLoader.TryLoad(path, out _, out var diagnostics);
                Assert.That(ok, Is.True);
                Assert.That(diagnostics.Warnings.Exists(w => w.Contains("MIQ_UNKNOWN_SECTION")), Is.True);
                Assert.That(diagnostics.WarningCodes, Does.Contain("MIQ_UNKNOWN_SECTION"));
            }
            finally
            {
                File.Delete(path);
        ...
        public void TryLoad_UnknownSection_IgnorePolicy_AllowsWithoutWarning()
        {
            ...
            try
            {
                var ok = MiqRuntimeLoader.TryLoad(
                    ...
                    out _,
                    out var diagnostics,
                    new MiqLoadOptions { UnknownSectionPolicy = MiqUnknownSectionPolicy.Ignore });
                Assert.That(ok, Is.True);
                Assert.That(diagnostics.Warnings.Exists(w => w.Contains("MIQ_UNKNOWN_SECTION")), Is.False);
                Assert.That(diagnostics.WarningCodes, Does.Not.Contain("MIQ_UNKNOWN_SECTION"));
            }
            finally
            {
                File.Delete(path);
        ...
        public void TryLoad_UnknownSection_FailPolicy_Fails()
        {
            ...
            try
            {
                var ok = MiqRuntimeLoader.TryLoad(
                    ...
                    out _,
                    out var diagnostics,
                    new MiqLoadOptions { UnknownSectionPolicy = MiqUnknownSectionPolicy.Fail });
                Assert.That(ok, Is.False);
                Assert.That(diagnostics.ErrorCode, Is.EqualTo(MiqLoadErrorCode.UnknownSectionNotAllowed));
            }
            finally
            {
                File.Delete(path);
        ...
        {
            var path = WriteTempFile(BuildValidMiqBytes(addPhysicsSections: true));
            try
            {
                var ok = MiqRuntimeLoader.TryLoad(path, out var payload, out var diagnostics);
                Assert.That(ok, Is.True);
                ...
                Assert.That(diagnostics.WarningCodes, Does.Contain("MIQ_PHYSICS_COMPONENT_UNAVAILABLE"));
            }
            finally
            {
                File.Delete(path);
        ...
        {
            var path = WriteTempFile(BuildValidMiqBytes(addPhysicsSections: true, missingPhysicsColliderRef: true));
            try
            {
                var ok = MiqRuntimeLoader.TryLoad(path, out _, out var diagnostics);
                Assert.That(ok, Is.True);
                Assert.That(diagnostics.WarningCodes, Does.Contain("MIQ_PHYSICS_REF_MISSING"));
            }
            finally
            {
                File.Delete(path);
        ...
        {
            var path = WriteTempFile(BuildValidMiqBytes(missingTextureRef: true));
            try
            {
                var ok = MiqRuntimeLoader.TryLoad(
                    path,
        ...
        private static byte[] BuildValidMiqBytes(
            ...
            bool typedAppendTrailingJunk = false)
        {
            ...
            {
                if (!MiqLz4Codec.TryCompress(meshPayload, out var compressed, preferRatio: true))
                {
        ...
        {
            var path = Path.Combine(Path.GetTempPath(), $"miq-test-{Guid.NewGuid():N}.miq");
