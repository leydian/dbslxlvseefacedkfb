...
{
    public static class MiqCiSmoke
    {
        [Serializable]
        ...
        public static void Run()
        {
            ...
            try
            {
                ...
                var outputPath = GetArgValue(args, "-miqSmokeOutputPath");
                reportPath = GetArgValue(args, "-miqSmokeReportPath");
                ...
                {
                    outputPath = Path.Combine(Path.GetTempPath(), "miq-ci-smoke.miq");
                }
                ...
                var options = new MiqExportOptions();
                options.FailOnMissingShader = false;
                MiqExporter.Export(outputPath, avatarRoot, options);
                ...
                {
                    throw new InvalidOperationException("MIQ export smoke failed to create output.");
                }

                MiqLoadDiagnostics diagnostics;
                MiqAvatarPayload payload;
                var ok = MiqRuntimeLoader.TryLoad(outputPath, out payload, out diagnostics);
                ...
                report.load_smoke_passed =
                    ok &&
                    diagnostics.ErrorCode == MiqLoadErrorCode.None &&
                    string.Equals(diagnostics.ParserStage, "runtime-ready", StringComparison.Ordinal);

                if (!report.load_smoke_passed)
                {
                    throw new InvalidOperationException(
                        "MIQ load smoke failed. code='" + diagnostics.ErrorCode + "', stage='" + diagnostics.ParserStage + "'");
                }
