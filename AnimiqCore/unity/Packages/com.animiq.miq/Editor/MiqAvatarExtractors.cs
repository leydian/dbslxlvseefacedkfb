...
{
    public interface IMiqAvatarExtractor
    {
        MiqAvatarPayload Extract(GameObject avatarRoot, MiqExportOptions options);
    }

    // Uses the scene AvatarRoot shape produced by UniVRM import workflows.
    public sealed class UniVrmAvatarExtractor : IMiqAvatarExtractor
    {
        ...
        public MiqAvatarPayload Extract(GameObject avatarRoot, MiqExportOptions options)
        {
            if (avatarRoot == null)
            {
            ...
            foreach (var smr in renderers)
            {
                ...
                for (var subMeshIndex = 0; subMeshIndex < subMeshCount; subMeshIndex++)
                {
                    ...
                    var meshPayload = new MiqMeshPayload
                    {
                        Name = meshName,
        ...
        private static MiqSkinPayload BuildSkinPayload(string meshName, Mesh mesh, SkinnedMeshRenderer smr)
        {
            ...
            var payload = new MiqSkinPayload
            {
                MeshName = meshName
            };
        ...
        private static MiqSkeletonPayload BuildSkeletonPayload(string meshName, Mesh mesh, SkinnedMeshRenderer smr)
        {
            var bindPoses = mesh.bindposes;
            ...
            var payload = new MiqSkeletonPayload
            {
                MeshName = meshName,
        ...
        private static MiqSkeletonRigPayload BuildSkeletonRigPayload(string meshName, SkinnedMeshRenderer smr)
        {
            var bones = smr.bones ?? Array.Empty<Transform>();
            if (bones.Length == 0)
            {
        ...
        private static MiqBlendShapePayload BuildBlendShapePayload(string meshName, Mesh mesh)
        {
            if (mesh.blendShapeCount <= 0)
            {
                return null;
            }
            var payload = new MiqBlendShapePayload { MeshName = meshName };
            ...
            for (var shapeIndex = 0; shapeIndex < mesh.blendShapeCount; shapeIndex++)
            {
                ...
                for (var frameIndex = 0; frameIndex < frameCount; frameIndex++)
                {
                    ...
                    payload.Frames.Add(new MiqBlendShapeFramePayload
                    {
        ...
        private static void ExtractPhysicsMetadata(Transform root, MiqAvatarPayload payload)
        {
        ...
        private static MiqPhysicsColliderPayload ExtractColliderPayload(Component component, Transform root)
        {
            ...
            var payload = new MiqPhysicsColliderPayload
            {
            ...
            {
                payload.Shape = MiqPhysicsColliderShape.Sphere;
            }
            else if (typeName.IndexOf("Capsule", StringComparison.OrdinalIgnoreCase) >= 0)
            {
                payload.Shape = MiqPhysicsColliderShape.Capsule;
            }
            else if (typeName.IndexOf("Plane", StringComparison.OrdinalIgnoreCase) >= 0)
            {
                payload.Shape = MiqPhysicsColliderShape.Plane;
            }
            else
            {
                payload.Shape = MiqPhysicsColliderShape.Unknown;
            }

            return payload;
        }

        private static MiqSpringBonePayload ExtractSpringBonePayload(
            Component component,
            Transform root,
            IReadOnlyDictionary<int, string> colliderNameByInstance)
        {
            ...
            return new MiqSpringBonePayload
            {
        ...
        private static MiqPhysBonePayload ExtractPhysBonePayload(
            Component component,
            Transform root,
            IReadOnlyDictionary<int, string> colliderNameByInstance)
        {
            ...
            HashSet<string> textureNameSet)
        {
            ...
            var baseTextureName = EnsureTextureRefForProperty(
                ...
                "_BaseMap",
                "_BaseColorMap");
            var item = new MiqMaterialPayload
            {
        ...
        private static bool HasTypedColor(MiqMaterialPayload payload, string id)
        {
            return payload.TypedColorParams.Exists(p => string.Equals(p.Id, id, StringComparison.Ordinal));
        }

        private static void AddTypedFloat(MiqMaterialPayload payload, Material material, string id, params string[] sourceProps)
        {
            foreach (var prop in sourceProps)
            {
                ...
                payload.TypedFloatParams.Add(new MiqTypedFloatParam
                {
                    Id = id,
        ...
        private static void AddTypedColor(MiqMaterialPayload payload, Material material, string id, params string[] sourceProps)
        {
            foreach (var prop in sourceProps)
            {
                if (!material.HasProperty(prop))
                ...
                payload.TypedColorParams.Add(new MiqTypedColorParam
                {
                    Id = id,
                    R = c.r,
                    G = c.g,
        ...
        private static void AddTypedTexture(MiqMaterialPayload payload, string slot, string textureRef)
        {
            if (string.IsNullOrWhiteSpace(textureRef))
            {
                return;
            }
            payload.TypedTextureParams.Add(new MiqTypedTextureParam
            {
                Slot = slot,
            ...
            params string[] propertyCandidates)
        {
            ...
            foreach (var prop in propertyCandidates)
            {
                ...
                if (encoded.Length == 0)
                {
                    Debug.LogWarning($"[MIQ] Texture encode failed for material '{material.name}', texture '{tex2d.name}'.");
                    continue;
                }
