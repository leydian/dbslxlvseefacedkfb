...
{
    public static class MiqImporter
    {
        public static MiqImportReport Import(string miqPath, MiqImportOptions options)
        {
            var report = new MiqImportReport
            {
                SourcePath = miqPath ?? string.Empty
            };

            if (string.IsNullOrWhiteSpace(miqPath) || !File.Exists(miqPath))
            {
                report.ErrorMessage = $"MIQ file not found: '{miqPath}'.";
                return report;
            }

            options ??= new MiqImportOptions();
            if (options.CollisionPolicy != MiqImportCollisionPolicy.Suffix)
            {
                report.ErrorMessage = $"Unsupported collision policy: {options.CollisionPolicy}";
                return report;
            }

            var loadOptions = new MiqLoadOptions
            {
                StrictValidation = options.StrictValidation,
                UnknownSectionPolicy = MiqUnknownSectionPolicy.Warn
            };

            if (!MiqRuntimeLoader.TryLoad(miqPath, out var payload, out var diagnostics, loadOptions))
            {
                report.ErrorMessage =
            ...
            var avatarId = SanitizeName(string.IsNullOrWhiteSpace(payload.Manifest.avatarId)
                ? Path.GetFileNameWithoutExtension(miqPath)
            ...
            MiqImportReport report)
        {
            ...
            for (var i = 0; i < payload.Textures.Count; i++)
            {
                ...
                {
                    report.Warnings.Add($"MIQ_IMPORT_TEXTURE_EMPTY: texture={src.Name}");
                    continue;
                }
                ...
                {
                    report.Warnings.Add($"MIQ_IMPORT_TEXTURE_WRITE_FAILED: texture={src.Name}, error={ex.Message}");
                    continue;
                }
                ...
                if (texture == null)
                {
                    report.Warnings.Add($"MIQ_IMPORT_TEXTURE_LOAD_FAILED: texture={src.Name}");
                    continue;
                }
            ...
            MiqImportOptions options)
        {
            ...
            for (var i = 0; i < payload.Materials.Count; i++)
            {
                ...
                {
                    report.RecoverableErrors.Add($"MIQ_IMPORT_MATERIAL_FAILED: material={src.Name}, error={ex.Message}");
        ...
        private static Dictionary<string, Mesh> CreateMeshAssets(MiqAvatarPayload payload, string meshesDir, MiqImportReport report)
        {
            ...
            var skinByMesh = new Dictionary<string, MiqSkinPayload>(StringComparer.OrdinalIgnoreCase);
            foreach (var skin in payload.Skins)
            {
                skinByMesh[NormalizeRef(skin.MeshName)] = skin;
            }

            var blendByMesh = new Dictionary<string, MiqBlendShapePayload>(StringComparer.OrdinalIgnoreCase);
            ...
            for (var i = 0; i < payload.Meshes.Count; i++)
            {
                ...
                {
                    report.RecoverableErrors.Add($"MIQ_IMPORT_MESH_FAILED: mesh={meshName}, error={ex.Message}");
                    report.SkippedAssets.Add($"mesh:{meshName}");
                }
            }
            ...
            MiqImportOptions options)
        {
            ...
            for (var i = 0; i < payload.Meshes.Count; i++)
            {
                ...
                {
                    report.Warnings.Add($"MIQ_IMPORT_MESH_INSTANCE_MISSING: mesh={meshPayload.Name}");
                    continue;
                }
                    ...
                    skin.BoneIndices.Length > 0)
                {
                    ...
                    else
                    {
                        ...
                        if (options.FailOnRigDataMissing || options.RigRecoveryPolicy == MiqRigRecoveryPolicy.Strict)
                        {
        ...
        private static Transform[] CreateSkeletonBonesFromRig(Transform parent, MiqSkeletonRigPayload rig, MiqImportReport report)
        {
        ...
        private static Transform[] MapSkinBones(Transform[] rigBones, int[] skinBoneIndices, MiqImportReport report, string meshName)
        {
        ...
        private static Transform ResolveRootBone(Transform[] bones, MiqSkeletonRigPayload rig, int[] skinBoneIndices, Transform fallback)
        {
        ...
        private static void ApplyPhysicsComponents(GameObject root, MiqAvatarPayload payload, MiqImportReport report)
        {
            ...
            for (var i = 0; i < payload.PhysicsColliders.Count; i++)
            {
                ...
                if (colliderType == null)
                {
                    ...
                    {
                        report.Warnings.Add("MIQ_PHYSICS_COMPONENT_UNAVAILABLE: collider-type-not-found");
                        report.WarningCodes.Add("MIQ_PHYSICS_COMPONENT_UNAVAILABLE");
                    }
                    continue;
                }
                ...
                {
                    report.Warnings.Add($"MIQ_PHYSICS_COMPONENT_UNAVAILABLE: collider={src.Name}, error={ex.Message}");
                    report.WarningCodes.Add("MIQ_PHYSICS_COMPONENT_UNAVAILABLE");
                }
            }
            ...
            ISet<string> unavailableTypes)
        {
            ...
            if (springType == null)
            {
                ...
                {
                    report.Warnings.Add("MIQ_PHYSICS_COMPONENT_UNAVAILABLE: springbone-type-not-found");
                    report.WarningCodes.Add("MIQ_PHYSICS_COMPONENT_UNAVAILABLE");
                }
                return;
            }
            ...
            {
                report.Warnings.Add($"MIQ_PHYSICS_COMPONENT_UNAVAILABLE: springBone={src.Name}, error={ex.Message}");
                report.WarningCodes.Add("MIQ_PHYSICS_COMPONENT_UNAVAILABLE");
            }
        }
            ...
            ISet<string> unavailableTypes)
        {
            ...
            if (physType == null)
            {
                ...
                {
                    report.Warnings.Add("MIQ_PHYSICS_COMPONENT_UNAVAILABLE: physbone-type-not-found");
                    report.WarningCodes.Add("MIQ_PHYSICS_COMPONENT_UNAVAILABLE");
                }
                return;
            }
            ...
            {
                report.Warnings.Add($"MIQ_PHYSICS_COMPONENT_UNAVAILABLE: physBone={src.Name}, error={ex.Message}");
                report.WarningCodes.Add("MIQ_PHYSICS_COMPONENT_UNAVAILABLE");
            }
        }
        ...
        private static Mesh BuildMesh(MiqMeshPayload src, string meshName, MiqImportReport report)
        {
            if (src == null)
            {
                return null;
            ...
            if (stride < 48)
            {
                report.Warnings.Add($"MIQ_IMPORT_VERTEX_STRIDE_UNSUPPORTED: mesh={meshName}, stride={stride}");
                return null;
            }
            if (src.VertexBlob == null || src.VertexBlob.Length < stride || (src.VertexBlob.Length % stride) != 0)
            {
                report.Warnings.Add($"MIQ_IMPORT_VERTEX_BLOB_INVALID: mesh={meshName}");
                return null;
            }
            ...
            if (src.Indices == null || src.Indices.Length == 0)
            {
                ...
                report.Warnings.Add($"MIQ_IMPORT_INDICES_MISSING: mesh={meshName}");
            }
            else
            {
                ...
                {
                    report.Warnings.Add($"MIQ_IMPORT_INDICES_NON_TRIANGLE: mesh={meshName}");
        ...
        private static void ApplySkin(Mesh mesh, MiqSkinPayload skin, MiqImportReport report)
        {
        ...
        private static void ApplyBlendShapes(Mesh mesh, MiqBlendShapePayload blend, MiqImportReport report)
        {
            ...
            foreach (var frame in blend.Frames)
            {
                ...
                {
                    report.Warnings.Add($"MIQ_IMPORT_BLENDSHAPE_INVALID: mesh={blend.MeshName}, frame={frame.Name}");
                    continue;
                }
        ...
        private static BoneWeight[] ParseBoneWeights(byte[] blob, int vertexCount, MiqImportReport report, string meshName)
        {
            ...
            {
                report.Warnings.Add(
                    $"MIQ_IMPORT_SKIN_WEIGHT_COUNT_MISMATCH: mesh={meshName}, expected={vertexCount * stride}, actual={blob.Length}");
                return Array.Empty<BoneWeight>();
            }
        ...
        private static Shader ResolveShader(MiqMaterialPayload source, MiqImportReport report)
        {
            ...
            report.Warnings.Add($"MIQ_IMPORT_SHADER_FALLBACK: material={source.Name}, shader={source.ShaderName}");
            if (shader == null)
            {
                report.RecoverableErrors.Add($"MIQ_IMPORT_SHADER_UNRESOLVED: material={source.Name}");
            }
            ...
            MiqImportOptions options)
        {
            ...
            {
                report.Warnings.Add($"MIQ_IMPORT_TEXTURE_UNRESOLVED: material={source.Name}, texture={source.BaseColorTextureName}");
            }
        }
        ...
        private static void ApplyTypedColor(Material material, MiqTypedColorParam value)
        {
        ...
        private static void ApplyTypedFloat(Material material, MiqTypedFloatParam value)
        {
        ...
        private static void SetupMaterialAlphaMode(
            ...
            string alphaMode,
            float cutoff,
            MiqMaterialRecoveryProfile recoveryProfile)
        {
            if (material == null)
            {
                return;
            ...
            if (recoveryProfile == MiqMaterialRecoveryProfile.Aggressive && material.HasProperty("_Blend"))
            {
            ...
            string message)
        {
            ...
            report.RigDiagnostics.Add(new MiqRigDiagnostic
            {
        ...
        {
            var normalized = string.IsNullOrWhiteSpace(path) ? "Assets/ImportedMiq" : path.Trim();
