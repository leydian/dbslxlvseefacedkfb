...
#if defined(_WIN32)
#if defined(ANIMIQ_NATIVECORE_EXPORTS)
#define ANIMIQ_API __declspec(dllexport)
#else
#define ANIMIQ_API __declspec(dllimport)
#endif
#else
#define ANIMIQ_API
#endif
...
ANIMIQ_API NcResultCode nc_initialize(const NcInitOptions* options);
ANIMIQ_API NcResultCode nc_shutdown(void);

ANIMIQ_API NcResultCode nc_load_avatar(const NcAvatarLoadRequest* request, NcAvatarHandle* out_handle, NcAvatarInfo* out_info);
ANIMIQ_API NcResultCode nc_unload_avatar(NcAvatarHandle handle);
ANIMIQ_API NcResultCode nc_get_avatar_info(NcAvatarHandle handle, NcAvatarInfo* out_info);
ANIMIQ_API NcResultCode nc_get_expression_count(NcAvatarHandle handle, uint32_t* out_count);
ANIMIQ_API NcResultCode nc_get_expression_infos(NcAvatarHandle handle, NcExpressionInfo* out_infos, uint32_t capacity, uint32_t* out_written);
ANIMIQ_API NcResultCode nc_get_springbone_info(NcAvatarHandle handle, NcSpringBoneInfo* out_info);
ANIMIQ_API NcResultCode nc_get_avatar_runtime_metrics_v2(NcAvatarHandle handle, NcAvatarRuntimeMetricsV2* out_info);

ANIMIQ_API NcResultCode nc_set_tracking_frame(const NcTrackingFrame* frame);
ANIMIQ_API NcResultCode nc_set_expression_weights(const NcExpressionWeight* weights, uint32_t count);
ANIMIQ_API NcResultCode nc_create_render_resources(NcAvatarHandle handle);
ANIMIQ_API NcResultCode nc_destroy_render_resources(NcAvatarHandle handle);
ANIMIQ_API NcResultCode nc_render_frame(const NcRenderContext* ctx);
ANIMIQ_API NcResultCode nc_create_window_render_target(const NcWindowRenderTarget* target);
ANIMIQ_API NcResultCode nc_resize_window_render_target(const NcWindowRenderTarget* target);
ANIMIQ_API NcResultCode nc_destroy_window_render_target(void* hwnd);
ANIMIQ_API NcResultCode nc_render_frame_to_window(void* hwnd, float delta_time_seconds);
ANIMIQ_API NcResultCode nc_render_avatar_thumbnail_png(const NcThumbnailRequest* request);
ANIMIQ_API NcResultCode nc_set_render_quality_options(const NcRenderQualityOptions* options);
ANIMIQ_API NcResultCode nc_get_render_quality_options(NcRenderQualityOptions* out_options);
ANIMIQ_API NcResultCode nc_set_lighting_options(const NcLightingOptions* options);
ANIMIQ_API NcResultCode nc_get_lighting_options(NcLightingOptions* out_options);
ANIMIQ_API NcResultCode nc_set_pose_offsets(const NcPoseBoneOffset* offsets, uint32_t count);
ANIMIQ_API NcResultCode nc_clear_pose_offsets(void);
