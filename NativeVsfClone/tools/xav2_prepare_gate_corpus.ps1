param(
    [string]$SourceDir = ".",
    [string]$OutputDir = ".\build\gate_corpus\xav2",
    [int]$MinSampleCount = 10,
    [switch]$IncludeBuildArtifacts,
    [string]$ManifestOutPath = ".\build\gate_corpus\xav2\sample_manifest.json"
)
...
$all = Get-ChildItem -Path $resolvedSource -Recurse -File -Filter *.xav2 -ErrorAction SilentlyContinue
$selected = [System.Collections.Generic.List[object]]::new()
$seen = [System.Collections.Generic.HashSet[string]]::new([System.StringComparer]::OrdinalIgnoreCase)
foreach ($f in $all) {
    $full = $f.FullName
    if ($full -match '\\NativeVsfClone\\NativeVsfClone\\') {
        continue
    }
...
if ($selected.Count -lt $MinSampleCount) {
    throw "Not enough .xav2 samples found. required=$MinSampleCount found=$($selected.Count)"
}
...
$lines = @()
$lines += "XAV2 Gate Corpus Prepared"
