...
- `.xav2` input extension validation path added in input validator (`R02` compatibility path completion)
...
- Avatar file dialog filter expanded to include `.xav2` (`R02`)
...
```powershell
powershell -ExecutionPolicy Bypass -File NativeVsfClone\tools\avatar_batch_validate.ps1 -InputDir sample
powershell -ExecutionPolicy Bypass -File NativeVsfClone\tools\release_gate_dashboard.ps1 -ReportDir NativeVsfClone\build\reports
powershell -ExecutionPolicy Bypass -File D:\dbslxlvseefacedkfb\NativeVsfClone\tools\version_contract_check.ps1 -HostCoreProject D:\dbslxlvseefacedkfb\NativeVsfClone\host\HostCore\HostCore.csproj -UnityPackageJson D:\dbslxlvseefacedkfb\NativeVsfClone\unity\Packages\com.vsfclone.xav2\package.json -OutputTxt D:\dbslxlvseefacedkfb\NativeVsfClone\build\reports\version_contract_check.txt
```
...
- `R15`: existing Unity XAV2 validation pipeline remains the SDK validator baseline in this round
