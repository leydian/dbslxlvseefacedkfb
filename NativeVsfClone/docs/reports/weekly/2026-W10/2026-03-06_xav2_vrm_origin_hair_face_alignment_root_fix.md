# XAV2 VRM-Origin Hair/Face Alignment Root Fix (2026-03-06)

## Summary
- Addressed a user-facing regression where VRM-origin `.xav2` avatars rendered with hair/face positional desync.
- Confirmed this was not a stale runtime DLL issue and not resolved by simple re-export alone.
- Final stable state was achieved by locking VRM-origin XAV2 out of runtime re-skinning paths.

## Symptoms and Context
- Observed in WPF preview/output after loading `개인작10-2.xav2`.
...
2. Re-export path validated:
   - ran `vrm_to_xav2` on `sample\\개인작10-2.vrm`
   - replaced root `개인작10-2.xav2` with re-export output
...
3. Source differential check:
   ...
   - XAV2 load did not carry equivalent runtime warning context, despite full payload counts.
4. Runtime policy isolation:
   - tested combinations of static skinning on/off and arm-pose path on/off
   - stable success only when VRM-origin XAV2 re-skinning paths were both disabled.

## Implemented Fix
- File: `src/nativecore/native_core.cpp`
- Policy locks:
  - `ShouldApplyStaticSkinningForAvatarMeshes(...)`
    - force `false` for `source_type == Xav2 && source_ext == ".vrm"`
  - `ShouldApplyArmPoseForAvatar(...)`
    - force `false` for `source_type == Xav2 && source_ext == ".vrm"` in auto mode
...
- Tooling checks:
  ...
  - `avatar_tool.exe "<xav2>" --dump-warnings-limit=200` -> PASS
- User validation:
  - final confirmation: "이제 잘돼!"

## Follow-up
- Keep current lock as safe default for VRM-origin XAV2 until transform metadata can be persisted/replayed explicitly through exporter + loader contract.
- Preferred long-term direction:
  - encode transformed-node/mesh-space correction metadata in XAV2
