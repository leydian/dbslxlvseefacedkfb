# XAV2 Unity SDK Diagnostics + Gate Expansion Report (2026-03-03)

## Scope

This update focuses on the Unity-side XAV2 SDK hardening track and gate stability:

- Add a non-throwing runtime loader API with structured diagnostics.
- Strengthen runtime parser boundary/schema validation while preserving XAV2 v1 compatibility.
- Improve exporter manifest default/required-field hygiene.
- Expand fixed-valid XAV2 gate input coverage via VRM-derived generation.
...
- Added `Xav2LoadErrorCode` with explicit categories:
  - `None`
  - `IoError`
  - `MagicMismatch`
  - `UnsupportedVersion`
...
- Added `Xav2LoadDiagnostics`:
  - `ErrorCode`
  - `ErrorMessage`
  - `ParserStage`
  - `IsPartial`
  - `Warnings`
- Added `Xav2RuntimeLoader.TryLoad(path, out payload, out diagnostics)`:
  - returns `false` instead of throwing on parse failure.
  - records stage-aware diagnostics (`parse`, `resolve`, `payload`, `runtime-ready`).
- Preserved `Xav2RuntimeLoader.Load(path)` behavior:
...
### 4) Gate input expansion from VRM-derived fixed XAV2 samples
...
- Added `-FixedXav2FromVrmCount` (default `5`) to sample report script.
- Added `Add-GeneratedXav2FromVrm` helper:
  - selects up to N `.vrm` samples.
  - generates `.xav2` via `vrm_to_xav2`.
  - registers generated files as `fixed-valid` XAV2 inputs.
- Added matching pass-through option in quality gate script.
- Existing synthetic-corrupt XAV2 generation and Gate F/G contracts remain unchanged.
...
- Added fixed XAV2 generation policy note in VXAvatar/VXA2/XAV2 gate section.
