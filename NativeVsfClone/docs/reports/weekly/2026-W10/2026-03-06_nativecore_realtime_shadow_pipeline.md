...
이 문서는 VSF 전용 실시간 그림자 세팅 참고 이미지를 기준으로, NativeVsfClone 런타임에 실시간 그림자 패스를 추가하고 Host/WPF/WinUI/Unity Exporter까지 동기화한 변경을 정리한다.  
범위는 기능 구현과 로컬 빌드 검증까지이며, WinUI XAML 컴파일 환경 이슈의 완전 해소는 범위에서 제외한다.

## Implemented Changes

1. Native API 확장 (`include/vsfclone/nativecore/api.h`)
