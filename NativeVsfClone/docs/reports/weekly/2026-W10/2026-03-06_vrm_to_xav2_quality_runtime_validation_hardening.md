# VRM -> XAV2 품질/런타임 검증 고도화 (실행 상세)

## 요약
- `vrm_to_xav2`에 품질 우선 검증을 강화하고, 변환 산출물을 런타임 경로로 재로딩해 진단 JSON에 `runtimeValidation`/`qualityGate`를 추가했다.
...
### 1) 변환기 품질 검증 강화 (`tools/vrm_to_xav2.cpp`)
- Rig 검증 추가/강화
...
- BlendShape 검증 추가
  - `XAV2_BLENDSHAPE_DELTA_VERTEX_INVALID`: vertex delta 바이트 정합 오류(12의 배수 아님)
  - `XAV2_BLENDSHAPE_DELTA_NORMAL_MISMATCH`: normal delta 크기 불일치
  - `XAV2_BLENDSHAPE_DELTA_TANGENT_MISMATCH`: tangent delta 크기 불일치
- Strict 정책 강화
  - P0 성격 코드들을 hard-reject 세트로 분리해 allowlist가 있어도 거부하도록 구성

### 2) 런타임 재검증 + 진단 계약 확장 (`tools/vrm_to_xav2.cpp`)
- 변환 직후 출력 XAV2를 `AvatarLoaderFacade`로 다시 로딩하는 `RuntimeValidation` 단계 추가
...
### 3) 품질 게이트 확장 (`tools/vrm_to_xav2_quality_gate.ps1`)
- `GateQ (quality)` 추가
...
## 검증 실행
- 빌드
  - `cmake --build build --config Release --target vrm_to_xav2 avatar_tool`
- 게이트
  - `powershell -ExecutionPolicy Bypass -File .\tools\vrm_to_xav2_quality_gate.ps1 -SampleDir ..\sample -MaxSamples 1`
- 결과 요약(`build/reports/vrm_to_xav2_quality_gate_summary.txt`)
