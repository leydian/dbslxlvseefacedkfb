# XAV2 Pass Flags Recovery + Strict Tracking Wrapper Follow-up (2026-03-06)
...
- XAV2 static skinning safety default clarification in native runtime,
...
- Static skinning auto-mode policy for XAV2 is explicitly safety-first:
  - default remains OFF unless explicitly forced.
- Added pass-flag fail-safe for XAV2 material path:
  - when all pass flags resolve to disabled (`base/depth/shadow/outline/emission`),
  - runtime forces `base` pass ON,
  - adds fallback reason `xav2_pass_flags_defaulted_to_base`.
