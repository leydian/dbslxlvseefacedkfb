# XAV2 static skinning auto-mode recovery (2026-03-06)

## Summary

This update addresses a runtime-visible XAV2 mesh breakage case where avatar load succeeds but render output appears collapsed/torn due to static skinning being disabled while skin payloads are present.
...
- default auto behavior now enables static skinning for XAV2 only when both skin and skeleton payloads are available
...
This conservative guardrail prevented risky rewrites, but also blocked required deformation for XAV2 assets that depend on skin payload application for sane render geometry.
...
1. Added explicit env mode resolution for `VSFCLONE_XAV2_ENABLE_STATIC_SKINNING`
...
2. Added avatar-aware effective policy function
   ...
   - behavior:
     ...
     - auto:
       - XAV2: on when both `skin_payloads` and `skeleton_payloads` exist
       - others: off
...
- Default runtime behavior improves for affected XAV2 avatars:
