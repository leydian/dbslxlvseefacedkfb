...
- XAV2 gate corpus preparation (10-sample baseline manifest generation)
...
### 4) XAV2 gate corpus preparation automation
...
- discovers `.xav2` files from a source root
- materializes a curated 10-sample corpus into `build/gate_corpus/xav2`
- emits gate manifest:
...
- `-EnableXav2CorpusPrep`
- `-Xav2CorpusSourceDir`
- `-Xav2CorpusOutputDir`
- `-Xav2CorpusMinSampleCount`
- `-Xav2CorpusIncludeBuildArtifacts`
...
- `powershell -ExecutionPolicy Bypass -File .\NativeVsfClone\tools\winui_xaml_min_repro.ps1 -NoRestore`
  - `FailureClass: TOOLCHAIN_XAML_PLATFORM_UNSUPPORTED`
  - `WMC9999Count: 2`
  - first diagnostic path and line captured
- `powershell -ExecutionPolicy Bypass -File .\NativeVsfClone\tools\winui_blocker_triage.ps1 -NoRestore`
  - summary + json emitted
- `powershell -ExecutionPolicy Bypass -File .\NativeVsfClone\tools\winui_diag_matrix_summary.ps1`
  - matrix summary + json emitted
- `powershell -ExecutionPolicy Bypass -File .\NativeVsfClone\tools\xav2_prepare_gate_corpus.ps1 -SourceDir . -OutputDir .\build\gate_corpus\xav2 -MinSampleCount 10 -IncludeBuildArtifacts`
  - `SampleCount: 10`
  - manifest emitted
- `powershell -ExecutionPolicy Bypass -File .\NativeVsfClone\tools\onboarding_kpi_calibrate.ps1 -TelemetryPath .\build\reports\telemetry_latest.json`
  - calibration summary + json emitted
- `powershell -ExecutionPolicy Bypass -File .\NativeVsfClone\tools\host_onboarding_state_smoke.ps1`
  - `Overall: PASS`
- `powershell -ExecutionPolicy Bypass -File .\NativeVsfClone\tools\release_gate_dashboard.ps1`
...
2. Execute Unity XAV2 validate/compression/parity gates once `UNITY_XAV2_PROJECT_PATH` is available.
