...
- Updated arm pose gate to use the same per-avatar static-skinning policy and removed XAV2 forced skip.
...
2. That function only returned true for explicit force-on env (`VSFCLONE_XAV2_ENABLE_STATIC_SKINNING=1|true|yes|on`).
3. Under default/auto runtime, arm pose apply path was skipped even when mesh static skinning was active.
4. Additional XAV2-only early return unconditionally disabled arm pose for XAV2 avatars.
...
### 2) XAV2 forced skip removal
- Removed temporary XAV2 early return block.
- Impact:
  - XAV2 arm pose can now apply under enabled policy and valid rig/skeleton payload prerequisites.
...
- Static checks:
  ...
  - confirmed XAV2 early-return bypass removal in arm pose function.
  - confirmed warning code insertion path for policy-disabled case.

## Expected Runtime Behavior After Fix
- Arm sliders should produce visible upper-arm pose updates in VRM and XAV2 when policy is enabled/auto-resolved ON.
