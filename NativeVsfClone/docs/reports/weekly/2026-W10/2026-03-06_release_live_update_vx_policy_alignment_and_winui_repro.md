...
- default gate scope now validates active support path (`XAV2`) and skips legacy VX/VXA2 checks
...
- `ReleaseCandidateWpfOnly` remained `FAIL` due to XAV2 Unity gate rows still `NOT_RUN/MISSING`
...
2. execute XAV2 Unity validation/compression/parity gates in an environment with a resolvable Unity project path
