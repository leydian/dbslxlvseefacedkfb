# 2026-03-06 - Arm pose XAV2(VRM-origin) auto-policy split fix

## Summary
- Implemented a native runtime hotfix to restore visible arm raise/lower motion for WPF arm sliders when using XAV2 avatars exported from VRM.
- Split arm-pose apply policy from mesh static-skinning policy to avoid unintended arm-pose disablement in `auto` mode.
- Preserved existing mesh-safety behavior for VRM-derived XAV2 while allowing upper-arm runtime pose application when pose payload prerequisites are present.

## Problem Context
- Repro context:
  - host: `WpfHost`
  - avatar: XAV2 (`sourceExt=.vrm`)
...
- Existing runtime behavior before this fix:
  ...
  - In `auto` mode, VRM-derived XAV2 returned `false` in static-skinning policy to prevent mesh-space mismatch risk.
...
2. Safety intent mismatch:
   - mesh static-skinning policy intentionally conservative for `sourceExt=.vrm` in XAV2 auto mode.
...
- Env semantics retained:
  - `VSFCLONE_XAV2_ENABLE_STATIC_SKINNING=on|1|true|yes` => enable
...
- Mesh static-skinning conservative policy for VRM-derived XAV2 remains unchanged to minimize render regression risk.
...
- Build:
  ...
  - `dotnet build NativeVsfClone/host/WpfHost/WpfHost.csproj -c Release -v minimal` => PASS
...
- XAV2 avatars with VRM-origin metadata in `auto` mode:
