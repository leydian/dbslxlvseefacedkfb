# XAV2 Shader Policy Relaxation + Runtime Fallback + CI Strict Lane (2026-03-06)
...
- Out of scope: format version/schema changes (`.xav2` wire shape), non-shader parity contracts, host/native runtime behavior outside Unity package.

## Implemented Changes

- Runtime load options expanded with shader policy selector.
  - Added `Xav2ShaderPolicy` with:
    - `WarnFallback` (default)
    - `Fail`
  - Added `Xav2LoadOptions.ShaderPolicy` defaulting to `WarnFallback`.
  - Code path: `unity/Packages/com.vsfclone.xav2/Runtime/Xav2DataModel.cs`
...
- Runtime loader unsupported-shader handling now policy-driven.
  ...
  - New behavior:
    - `WarnFallback`: continue load, force material `ShaderFamily` to `standard`, append warning code `XAV2_SHADER_FAMILY_FALLBACK`.
    - `Fail`: preserve prior hard-fail behavior with `ParityContractViolation` + `CriticalParityViolation = true`.
  - Code path: `unity/Packages/com.vsfclone.xav2/Runtime/Xav2RuntimeLoader.cs`

- Editor export default UX changed to relaxed-first.
  - `Tools/VsfClone/XAV2/Export Selected AvatarRoot` now uses relaxed export options (`FailOnMissingShader=false`).
  - Strict path remains explicitly available as:
    - `Tools/VsfClone/XAV2/Export Selected AvatarRoot (Strict)`
  - Code path: `unity/Packages/com.vsfclone.xav2/Editor/Xav2ExportMenu.cs`

- CI parity probe explicitly pinned to strict shader policy.
  - `Xav2CiQuality.RunParityProbe()` now calls runtime loader with `ShaderPolicy = Fail`.
  - This keeps parity gate behavior strict even after default runtime policy became relaxed.
  - Code path: `unity/Packages/com.vsfclone.xav2/Editor/Xav2CiQuality.cs`
...
- Runtime tests updated for new contract.
  ...
  - Code path: `unity/Packages/com.vsfclone.xav2/Tests/Runtime/Xav2RuntimeLoaderTests.cs`
...
2. Execute `tools/xav2_parity_gate.ps1` and confirm strict lane still blocks unsupported shader samples.
3. Execute `tools/unity_xav2_validate.ps1` on the target Unity project and archive artifacts in `build/reports`.
