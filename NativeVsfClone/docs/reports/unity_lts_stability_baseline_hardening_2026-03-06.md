...
`tools/unity_xav2_lts_gate.ps1` now treats all three lines as official by default:
...
- `build/reports/unity_xav2_lts_gate_history.csv`
- `build/reports/unity_xav2_lts_kpi_summary.json`
- `build/reports/unity_xav2_lts_kpi_summary.txt`
...
- read `unity_xav2_lts_gate_summary.json` when available
- show `Unity XAV2 LTS Gate (Overall)`
- show per-line rows (e.g., `Unity XAV2 LTS Line [2022-lts]`)
...
1. Run `unity_xav2_lts_gate.ps1` on CI runners with all three editor env vars configured and archive first KPI baseline.
