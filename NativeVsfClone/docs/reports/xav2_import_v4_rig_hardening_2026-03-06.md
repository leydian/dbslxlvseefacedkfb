# XAV2 Import v4 Rig Hardening Update (2026-03-06)

## 1) 목적

기존 Unity XAV2 Import 경로는 "기본 복원"은 가능했지만, 스키닝 본 계층을 더미 본으로 생성해 고정밀 리깅 복원 정확도가 낮았다.
이번 변경은 다음을 목표로 진행했다.

- XAV2 포맷을 v4로 확장해 리깅 계층 정보를 명시적으로 포함
...
- Runtime 데이터 모델에 추가
  - `Xav2RigBonePayload`
  - `Xav2SkeletonRigPayload`
  - `Xav2AvatarPayload.SkeletonRigs`

### 2.2 Export 경로

- Export 버전을 `3 -> 4`로 상향
- `Xav2AvatarExtractors`에서 `SkinnedMeshRenderer.bones` 기반 rig payload 생성
  - bone 이름
  - parent index
  - 로컬 TRS 행렬(4x4)
- `Xav2Exporter`에서 `0x0017` 섹션 직렬화
...
- `Xav2RuntimeLoader`가 `0x0017` 파싱 추가
- v4 스키닝 호환성 평가 규칙 추가
...
- 포맷 문서(`docs/formats/xav2.md`)에 v4/0x0017 명세 추가
