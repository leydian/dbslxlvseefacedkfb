# XAV2 10/10 Gate Hardening Execution Report (2026-03-06)

## Scope

This report captures the implementation and validation status of the XAV2 "10/10" hardening baseline:
...
### 1) Warning-code normalization in native XAV2 loader
...
- For `W_*` / `E_*` records, warning-code extraction now prefers the stable second token (e.g. `XAV2_*`/`XAV3_*`) when present.
...
- Current workspace still has only one `.xav2` sample in gate scope; strict baseline cannot pass until sample corpus is expanded.
...
1. Populate and register at least 10 curated `.xav2` samples (normal/boundary/corrupt) and enable `-FailOnManifestMismatch`.
