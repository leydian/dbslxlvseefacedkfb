...
- Existing XAV2 conservative/fallback behavior is preserved and still emits:
...
- `cmake --build NativeVsfClone/build-thumb --config Release --target nativecore avatar_tool`
  - result: PASS

## Runtime probe

- `NativeVsfClone/build-thumb/Release/avatar_tool.exe sample/개인작11-3.vrm`
  - result: PASS
  - key observations:
    - `Format: VRM`
    - `MissingFeatures: 2`
...
- `NativeVsfClone/build-thumb/Release/avatar_tool.exe sample/Kikyo_FT Variant.vrm`
  - result: PASS
  - key observations:
