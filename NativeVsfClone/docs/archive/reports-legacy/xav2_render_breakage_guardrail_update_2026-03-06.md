# XAV2 render breakage guardrail update (2026-03-06)

## Summary

This update applies a focused stabilization pass for the XAV2/lilToon render-breakage line by reducing high-risk deformation behavior in native render, normalizing typed material texture resolution, and strengthening diagnostics parity between native and Unity runtime loader paths.
...
Recent XAV2 changes introduced:
...
- Added opt-in toggle for static skinning application:
  - env var: `VSFCLONE_XAV2_ENABLE_STATIC_SKINNING`
...
- When skin payload exists but static skinning is disabled, native path now records warning:
  - `W_RENDER: XAV2_SKINNING_STATIC_DISABLED: ...`
  - warning code: `XAV2_SKINNING_STATIC_DISABLED`
...
- When typed `base` texture reference is present but unresolved, native path emits:
  - `W_RENDER: XAV2_MATERIAL_TYPED_TEXTURE_UNRESOLVED: ...`
  - warning code: `XAV2_MATERIAL_TYPED_TEXTURE_UNRESOLVED`
...
- Added post-material validation pass:
  ...
  - emits warning for unresolved refs:
    - `W_PAYLOAD: XAV2_MATERIAL_TYPED_TEXTURE_UNRESOLVED: ...`
...
- Sample XAV2 load remained healthy:
  - `Load succeeded`
  - `Format: XAV2`
  - `Compat: full`
