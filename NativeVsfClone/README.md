...
Native runtime workspace for loading avatar formats (`.vrm`, `.xav2`, `.vsfavatar`) and running host apps (`WPF`, `WinUI`) on top of `nativecore.dll`.
...
```powershell
cmake -S . -B build -G "Visual Studio 17 2022" -A x64 -DVSFCLONE_ENABLE_SPOUT2=ON
cmake --build build --config Release
```

Spout2 SDK note:

- if `third_party/Spout2/include` exists, native build enables `VSFCLONE_SPOUT2_ENABLED`
- if not found, build falls back to legacy shared-memory sender automatically

Run quick probes:

```powershell
.\build\Release\vsfclone_cli.exe "D:\path\to\avatar.vsfavatar"
...
- Avatar load/query/unload (`.vrm`, `.xav2`, `.vsfavatar`)
...
- `include/vsfclone/`: public interfaces
- `src/`: runtime and loaders
- `host/`: WPF/WinUI apps + shared HostCore
- `unity/Packages/com.vsfclone.xav2/`: Unity package scaffold
